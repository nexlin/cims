...
/**
 * CmpLog.h — CMP 서버 체계적 로깅 유틸리티 (Header-Only)
 *
 * 기능:
 ...
 * 사용법:
 *   // 초기화 (파일 로깅)
 *   CmpLog::Instance().InitFile("../log", "cmp", 10, 5);  // 10MB, 5개 파일
 *
...
public:
    ...
    void InitFile(const std::string& logDir, const std::string& baseName,
        ...
        if (!_fp) {
            fprintf(stderr, "[CmpLog] Failed to open log file: %s\n", _currentPath.c_str());
        }
    }
...
private:
    CmpLog() : _level(CMP_LOG_INFO), _fp(NULL), _maxSizeBytes(10L * 1024 * 1024), _maxFiles(5), _written(0) {}
...
#define LOG_ERROR(tag, fmt, ...) \
    CmpLog::Instance().Print(CMP_LOG_ERROR, tag, fmt, ##__VA_ARGS__)

#define LOG_WARN(tag, fmt, ...) \
    CmpLog::Instance().Print(CMP_LOG_WARN, tag, fmt, ##__VA_ARGS__)

#define LOG_INFO(tag, fmt, ...) \
    CmpLog::Instance().Print(CMP_LOG_INFO, tag, fmt, ##__VA_ARGS__)

#define LOG_DEBUG(tag, fmt, ...) \
    CmpLog::Instance().Print(CMP_LOG_DEBUG, tag, fmt, ##__VA_ARGS__)
...
#define LOG_THROTTLE(intervalSec, tag, fmt, ...) \
    CmpLog::Instance().PrintThrottled(intervalSec, CMP_LOG_INFO, tag, fmt, ##__VA_ARGS__)
