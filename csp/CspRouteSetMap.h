...
struct RouteSetRuntime {
    ...
    RouteSetRuntime(const RouteSetRuntime& o)
        : rr_cursor(o.rr_cursor.load()),
          weighted_deficit(o.weighted_deficit.load()) {}
...
public:
    ...
    std::string SelectRoute(const std::string& routeSetName,
...
private:
    ...
    std::string _selectHashByCaller(const RouteSetEntry& e, const std::string& key,
                                    std::string& outReason);
};
