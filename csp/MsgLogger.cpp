...
void CMsgLogger::Log( const char* iface, const char* dir, const char* proto, const char* method, const char* peer ) {
    ...
    char line[512];
    snprintf( line, sizeof( line ), "{\"ts\":\"%s\",\"dir\":\"%s\",\"proto\":\"%s\",\"method\":\"%s\",\"peer\":\"%s\"}",
              ts.c_str(), dir ? dir : "", proto ? proto : "", method ? method : "", peer ? peer : "" );
...
bool CMsgLogger::EnsureDirRecursive( const std::string& path ) {
    ...
    if ( pos != std::string::npos && pos > 0 ) EnsureDirRecursive( path.substr( 0, pos ) );
