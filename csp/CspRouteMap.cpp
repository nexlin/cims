        ...
        for (size_t i = 0; i < items.Size(); ++i) {
            ...
            if (!c.IsValid()) {
                CLog::Print(LOG_ERROR,
                    "RouteMap: skip invalid record (name='%s' local='%s' remote='%s')",
            ...
            if (newPair.count(pairKey)) {
                CLog::Print(LOG_ERROR,
                    "RouteMap: duplicate (local='%s', remote='%s') pair — keeping first '%s', skipping '%s'",
                    c.local_node_ref.c_str(), c.remote_node_ref.c_str(),
                    newPair[pairKey].c_str(), c.name.c_str());
                continue;
            }
    ...
    for (auto& kv : m_byName) {
        ...
        if (!lnOk || !rnOk) {
            CLog::Print(LOG_ERROR,
                ...
                c.name.c_str(),
                c.local_node_ref.c_str(), lnOk ? "yes" : "no",
                c.remote_node_ref.c_str(), rnOk ? "yes" : "no");
            c.enabled = false;
            ++invalid;
        }
    }
...
RouteConfig CCspRouteMap::GetByPair(const std::string& localName,
...
bool CCspRouteMap::MarkAlive(const std::string& routeName, int rtt_ms) {
    ...
    if (!was) {
        CLog::Print(LOG_SYSTEM, "RouteMap: route '%s' went ALIVE (rtt=%dms)",
                    routeName.c_str(), rtt_ms);
    }
    return true;
}
