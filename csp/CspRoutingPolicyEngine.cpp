...
RoutingDecision CspRoutingPolicyEngine::Decide(const MessageCtx& ctx,
    ...
    for (const auto& p : snapshot) {
        ...
        if (p.target_type == "route_set") {
            ...
            CLog::Print(LOG_INFO,
                "RoutingPolicyEngine: '%s' matched but route_set '%s' unavailable (%s)",
        ...
        CLog::Print(LOG_ERROR,
            "RoutingPolicyEngine: policy '%s' unknown target_type '%s'",
            p.name.c_str(), p.target_type.c_str());
    }
