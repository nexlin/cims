...
void McDataArchiveMessage( const char *pszGroup, const char *pszFrom, const char *pszMsgType,
                           ...
                           const char *pszVia, const char *pszFileUrl, bool bMcData ) {
    ...
    snprintf( szEvt, sizeof( szEvt ),
              ...
              pszVia && pszVia[0] ? ",\"via\":\"" : "", pszVia ? pszVia : "",
              pszVia && pszVia[0] ? "\"" : "" );
    gclsCallDir.PttLogEvent( pszGroup, "message_sent", szEvt );

    // 메시지 보관 — {ServiceLogDir}/message/{gid}/{시간버킷}/messages.jsonl (콘솔 모니터링 SoT)
    std::string strRec = std::string( "{\"group\":\"" ) + _jesc( pszGroup ) + "\",\"from\":\"" +
                         _jesc( pszFrom ) + "\",\"msg_type\":\"" + pszMsgType + "\",\"conv_id\":\"" +
                         clsInfo.m_strConvId + "\",\"msg_id\":\"" + clsInfo.m_strMsgId + "\",\"text\":\"" +
                         _jesc( clsInfo.m_strText ) + "\",\"size\":" + std::to_string( iPayloadSize ) +
    ...
    bool bFileFields = ( clsInfo.m_iMsgType == MCDATA_MSG_FD_SIGNALLING ) ||
                       ( pszFileUrl && pszFileUrl[0] );
    if ( bFileFields ) {
        ...
        strRec += std::string( ",\"file_name\":\"" ) + _jesc( clsInfo.m_strFileName ) + "\",\"file_url\":\"" +
                  _jesc( strUrl ) + "\",\"file_size\":" + std::to_string( clsInfo.m_llFileSize ) +
