...
bool CspRuleEvaluator::LoadAll() {
            ...
            for (const auto& mem : rs.members) {
                if (newRules.find(mem.rule_ref) == newRules.end()) {
                    CLog::Print(LOG_ERROR,
                        "RuleEvaluator: rule_set '%s' references missing rule '%s'",
    ...
    CLog::Print(LOG_INFO,
        "RuleEvaluator: loaded rules=%zu rule_sets=%zu",
        RuleCount(), RuleSetCount());
    return true;
}
...
const std::string* CspRuleEvaluator::_getFieldValue(const MessageCtx& ctx,
...
bool CspRuleEvaluator::_applyOp(const std::string& fv, const std::string& op,
                                const std::string& val, bool fieldExists) const {
    ...
    if (op == "suffix")   return fv.size() >= val.size() &&
                                 fv.compare(fv.size() - val.size(), val.size(), val) == 0;
    ...
    if (op == "in_list") {
        ...
        for (const auto& it : items) if (it == fv) return true;
        return false;
    }
...
bool CspRuleEvaluator::_evalRule(const Rule& r, const MessageCtx& ctx) const {
    ...
    if (!fv) {
        CLog::Print(LOG_ERROR, "RuleEvaluator: rule '%s' unknown field '%s'",
...
bool CspRuleEvaluator::MatchRuleSet(const std::string& ruleSetName, const MessageCtx& ctx) const {
    ...
    if (it == m_ruleSets.end()) {
        CLog::Print(LOG_DEBUG, "RuleEvaluator: unknown rule_set '%s' — treating as no-match",
                    ruleSetName.c_str());
        return false;
    }
