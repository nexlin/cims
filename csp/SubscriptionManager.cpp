...
void CSubscriptionManager::AddSubscription( const std::string &strResourceUri, const SubscriptionInfo &info ) {
    ...
    if ( it != m_mapSubs.end() ) {
        ...
        CLog::Print( LOG_DEBUG, "Subscription Updated: User=%s Type=%s CallId=%s Expires=%d", info.strUserId.c_str(),
                     info.strEventType.c_str(), info.strCallId.c_str(), info.iExpires );
    } else {
        m_mapSubs[info.strCallId] = info;
        CLog::Print( LOG_INFO, "Subscription Added: User=%s Type=%s CallId=%s Expires=%d", info.strUserId.c_str(),
                     info.strEventType.c_str(), info.strCallId.c_str(), info.iExpires );
    }
}
...
void CSubscriptionManager::RemoveSubscription( const std::string &strCallId ) {
    ...
    if ( it != m_mapSubs.end() ) {
        CLog::Print( LOG_INFO, "Subscription Removed: User=%s Type=%s CallId=%s", it->second.strUserId.c_str(),
                     it->second.strEventType.c_str(), strCallId.c_str() );
        m_mapSubs.erase( it );
    }
}
...
void CSubscriptionManager::GetSubscriptionsByEvent( const std::string &strEventType,
                                                    std::list<SubscriptionInfo> &outList ) {
    std::unique_lock<std::recursive_mutex> lock( m_mutex );

    for ( auto &pair : m_mapSubs ) {
        if ( pair.second.strEventType == strEventType ) {
            outList.push_back( pair.second );
        }
    }
}
...
void CSubscriptionManager::CheckExpired() {
    ...
    for ( auto it = m_mapSubs.begin(); it != m_mapSubs.end(); ) {
        ...
        if ( now > tExpireAt ) {
            CLog::Print( LOG_INFO, "Subscription Expired: User=%s Type=%s CallId=%s", it->second.strUserId.c_str(),
                         it->second.strEventType.c_str(), it->first.c_str() );
