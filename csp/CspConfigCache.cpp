...
const char* kEntityName[CACHE_COUNT] = {
    "local_node", "remote_node", "route", "route_set",
    "rule", "rule_set", "routing_policy", "acl_policy",
    "access_service",
};
...
bool CCspConfigCache::_loadFromJsonl(CspCacheEntity e) {
    ...
    while (std::getline(ifs, line)) {
        ...
        if (rec.type != SimpleJson::JSON_OBJECT) {
            CLog::Print(LOG_ERROR, "CspConfigCache: %s jsonl line skipped (not object)",
                        kEntityName[e]);
            continue;
        }
        arr.Add(rec);
    ...
    CLog::Print(LOG_INFO, "CspConfigCache: loaded %s from %s (%d records)",
                kEntityName[e], path.c_str(), count);
    return true;
}
