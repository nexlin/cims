...
bool CFmReporter::Init( const std::string &strOamIp, int iOamPort, const std::string &strNode,
    ...
    if ( clsFile.is_open() ) {
        ...
        CLog::Print( LOG_INFO, "FmReporter: catalog loaded (%s)", strCatalogFile.c_str() );
    } else {
        CLog::Print( LOG_ERROR, "FmReporter: catalog open failed (%s) — 빈 카탈로그로 등록", strCatalogFile.c_str() );
    }

    m_hSocket = socket( AF_INET, SOCK_DGRAM, 0 );
    if ( m_hSocket < 0 ) {
        CLog::Print( LOG_ERROR, "FmReporter: socket error" );
        return false;
    }
    ...
    CLog::Print( LOG_SYSTEM, "FmReporter: started (oam=%s:%d, node=%s, boot_id=%lld, sync=%ds)", m_strOamIp.c_str(),
...
void CFmReporter::Stop() {
    ...
    CLog::Print( LOG_SYSTEM, "FmReporter: stopped" );
}
...
void CFmReporter::AlarmOpen( const std::string &strCode, const std::string &strMo,
    ...
    CLog::Print( LOG_INFO, "FmReporter: ALARM OPEN %s", strAkey.c_str() );
}
...
void CFmReporter::AlarmClose( const std::string &strCode, const std::string &strMo ) {
    ...
    nodePayload.Set( "ts", FmNowIso() );
    SendFm( "FM_ALARM", nodePayload );
    CLog::Print( LOG_INFO, "FmReporter: ALARM CLOSE %s", strAkey.c_str() );
}
...
unsigned int CFmReporter::SendFm( const std::string &strCmd, SimpleJson::JsonNode &nodePayload ) {
    ...
    if ( strPacket.size() > kFmMaxPacket ) {
        CLog::Print( LOG_ERROR, "FmReporter: %s packet too large (%zu > %zu) — 미전송", strCmd.c_str(),
...
void CFmReporter::HandleResponse( const SimpleJson::JsonNode &nodeMsg ) {
    ...
    if ( strStatus == "OK" ) {
        if ( strCmd == "FM_REGISTER" ) {
            ...
            CLog::Print( LOG_INFO, "FmReporter: registered (sync=%ds)", m_iSyncSec );
    ...
    if ( strCode == "UNREGISTERED" ) {
        ...
        if ( m_bRegistered ) CLog::Print( LOG_INFO, "FmReporter: OAM 미등록 응답 — 재등록 예정" );
        m_bRegistered = false;
    } else {
        CLog::Print( LOG_ERROR, "FmReporter: %s ERROR %s (%s)", strCmd.c_str(), strCode.c_str(),
        ...
        std::vector<std::string> vecResend;
        {
            ...
            for ( std::map<unsigned int, Pending>::iterator it = m_mapPending.begin(); it != m_mapPending.end(); ) {
                ...
                if ( it->second.iAttempts >= kFmMaxAttempts ) {
                    CLog::Print( LOG_DEBUG, "FmReporter: %s ack 미수신 %d회 — 폐기 (trans_id=%u)",
