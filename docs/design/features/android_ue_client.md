...
- **앱 UI 구조**(`ptt-client/ui/`, 시안 `android/assets/pages/` 기준 — 다크 배경+민트 액센트, 토큰=`ui/Theme.kt` `Ct`, 아이콘=`assets/svgs` 변환 VectorDrawable):
  ...
  - **전체채널 탭**(`ChannelsScreen`)=필터 칩(전체/참여 중/미참여)+채널 행(역할 배지·구성원 수·수신 중 배지·메시지 버튼), 행 탭→**채널 상세**(`ChannelDetailScreen`)=상태 안내 배너+채널 상태 카드+접속 구성원 목록(이니셜 아바타·송신 중 표시)+하단 [주채널 설정]/[부채널 설정]/[나가기].
