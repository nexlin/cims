...
- **앱 UI 구조**(`ptt-client/ui/`, 시안 `android/assets/pages/` 기준 — 다크 배경+민트 액센트, 토큰=`ui/Theme.kt` `Ct`, 아이콘=`assets/svgs` 변환 VectorDrawable):
  - 라우팅=`ui/AppRoot.kt`(`Nav` sealed: Splash→Home 4탭→Channel/Thread/Settings 푸시, BackHandler 계층 복귀). 컨트롤러 상태는 `PttUiState` 로 묶어 화면에 전달.
  - **스플래시**(`SplashScreen`)=동심원 로고+기관 로고(`yrt_logo`), CIMS SSO 라 별도 로그인 화면 없음 — 계정 있으면 등록 진행 후 자동 진입, 없으면 "CIMS 로그인 열기".
  - **주채널 탭**(`MainChannelScreen`)=주채널 카드(역할 배지+태그칩+발언 상태 패널(경과 타이머)+수신 음량 슬라이더(STREAM_VOICE_CALL)+가로형 PTT 바+메시지 버튼) + 부채널 카드들 + 모니터링 행 + 컨트롤 행(스피커/전체듣기/SOS 길게). PTT 바 상태색: 대기=민트/요청=황색/발언=민트+펄스/수신=회색면. 하드웨어 버튼 단말은 PTT 바 대신 안내 문구.
  - **전체채널 탭**(`ChannelsScreen`)=필터 칩(전체/참여 중/미참여)+채널 행(역할 배지·구성원 수·수신 중 배지·메시지 버튼), 행 탭→**채널 상세**(`ChannelDetailScreen`, 시안 `채널선택화면-상세.png`)=헤더(채널명+그룹 우선순위 박스+음성/영상 배지+CH 번호)+역할 안내 배너(주/부/일반 3케이스)+채널 상태 카드(구성원 N명·접속 중 M명·발언자)+구성원 목록+하단 케이스별 버튼(주채널=나가기만 / 부채널=[주채널 설정] / 일반=[주채널 설정]+[부채널 설정]).
  - **채널 상세 구성원 명부 = TS 24.481 그룹 문서(표준 필드만)**: 진입 시 `PttController.loadGroupDetail` 이 GMS XCAP `getGroupDoc`(ETag/If-None-Match 캐시)으로 그룹 문서를 받아 `csc/CscModels.kt GroupDoc.parse` 로 파싱 — 멤버별 `entry uri`(tel: = 전화번호)·`rl:display-name`(이름)·`mcpttgi:participant-type`(chair→"의장" 배지)·`mcpttgi:user-priority`(P1 붉은/P2~3 민트/그 외 회색 배지), 그룹 레벨 `on-network-group-priority`(헤더 우선순위 박스)·`mcptt-video`(음성/영상)·`session-type`·`max-participant-count`. 접속/발언 상태는 conference-info(RFC 4575) 참가자·floor(TS 24.380) 화자와 번호 키로 결합해 **접속 중(우선순위순)/오프라인** 섹션 분리(미참여 시엔 접속 여부를 알 수 없어 중립 "구성원" 명부). 직책 등 규격 외 필드는 그룹 문서에 넣지 않는다 — CSC 가 내려주는 정보는 표준 규격 준수.
  - **메시지 탭**(`MessagesScreen`)=스레드 목록(안읽음 배지)→대화(`MessageThreadScreen`)=날짜 구분+말풍선(발신=민트 우측/수신=다크 좌측)+입력바. 발신=`PttController.sendGroupMessage`(SIP MESSAGE, 그룹 URI — CSP fan-out), 수신=`SipController.incomingMessage`→core `MessageStore` 영속(`PttService`), 변경 틱=`PttService.messageTick`. 미디어 첨부는 후속 과제(서버 경로 미구현).
  - **통화이력 탭**(`HistoryScreen`)=일자 구분+이벤트 행(내/타인 발언(발언시간)·참여/종료·긴급 개시/수신/해제). 소스=`PttController.onEvent` 훅(`PttEvent`: JOIN/LEAVE/TALK_ME/TALK_OTHER/EMERGENCY*)→`ptt-client/history/HistoryStore.kt`(SharedPreferences JSON, 최대 500건).
  - **설정**(`SettingsScreen`, 주채널 탭 우상단 기어)=프로필(등록 상태·등록/해제)+통신 설정(스피커 출력/전체 듣기 토글)+하드웨어 버튼 설정(`KeyConfigOverlay` 열기)+그룹 새로고침·버전.
...
- **멀티그룹 동시 참여**(TS 22.179 group scanning): 한 단말이 N개 그룹에 동시 참여한다. 그룹별로 독립 SIP 다이얼로그+floor 소켓+FloorClient 를 가지며(`PttController.Session`), UI 는 주채널 탭 카드(주·부·모니터링)와 전체채널 목록으로 나열한다. **주채널(primary)/부채널(secondary)** 지정은 채널 상세/부채널 카드의 버튼(`setPrimary`/`toggleSecondary`), **듣기 정책**(주·부만 / 전체듣기)은 주채널 탭 컨트롤 행·설정 화면 토글로 그룹별 `setCallListen`(하향 오디오 mix on/off)을 적용한다. **PTT 발언은 주채널 세션만** 대상(`pttDown`/`pttUp`). 그룹별 나가기(채널 상세)·참여는 독립적. 발언 상태 패널은 주채널 기준으로 파생하되 비주채널 화자는 `[gNNN]` 태그를 붙인다. per-call 문맥 분리 필수: floor 목적지/conference 정보는 callId 로 구분, SDP 주입은 `CimsCall.pendingAppSdp`(전역이면 그룹 간 혼선).
