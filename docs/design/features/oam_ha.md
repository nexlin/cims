...
| 패키지 메타 + 타르볼 | `control/packages` + 패키지 저장소(사이트 `packages/` — 단일 루트면 store 의 `pkg_files`) | 설치·업그레이드·`/install-agent.sh` 불가 |
...
| `run/keepalived/ha.json`, `run/managed_ips.json`, `run/supervised.json` | `auth_codes`, `refresh_tokens`, `verify_runs` (카테고리 없이 store 루트 직하) + 패키지 저장소(사이트 `packages/`, 단일 루트면 store 직하 `pkg_files`) |
...
[시스템/인프라] 의 일이고 store 위치는 그냥 경로다 — 사이트 디렉터리(`CimsSiteDir`)의 `runtime/` 영역
([site_directory_layout.md](site_directory_layout.md)). 마운트 지점은 공유 스토리지 사이트가 mount guard 용으로
적는 **선택** 값이다.

```text
입력:  oam 배포설정 CimsSiteDir        ← 사이트 디렉터리 (부트스트랩 --site-dir / 콘솔 [패키지 설정] > oam > 사이트 디렉터리)
       oam 배포설정 CimsRuntimeDir     ← (선택) 관리 store override
       oam 배포설정 CimsRuntimeMount   ← (선택) 공유 스토리지 마운트 지점 — mount guard(§4.3) 기준.
          │                               사이트 디렉터리도 store 경로도 없으면 store = <마운트>/runtime.
          │                               셋 다 비면 노드 로컬 modules/oam/runtime (단일 루트 레이아웃).
          ├── 유도(services.paths) → 실효 store 경로 = <사이트>/runtime
          │                        → Packages.Dir = <사이트>/packages (단일 루트면 <store>/pkg_files)
          │                        → 콘텐츠·녹취·로그·통계·상태 영역 (site_directory_layout.md §3)
          │        실체화(agents._materialize_deploy_config)가 매 디스패치마다 채운다.
          │
          ├── 유도(agents._store_source) → oam-svc·csc config.json (같은 store 를 읽는다)
          ├── 유도(agents._site_source)  → 서비스 모듈 config.json 의 영역 경로(템플릿 site_area)
          │
...
사이트 디렉터리·마운트를 비우면 = **노드 로컬**(`modules/oam/runtime`). 단일 노드·부트스트랩 직후의
...
- **`scope=service`(공통 설정)** 이다 — 멤버 간 반드시 동일해야 하는 값이므로 서버별 개별
  ...
  편집 창구는 **그룹 > 패키지 설정 > oam > 사이트 디렉터리** 하나이고, 입력은 사이트 디렉터리 하나다
  (영역 경로는 비워 두면 유도되고, 따로 적으면 그 값이 이긴다). R4 자동 교정이 ACTIVE 기준으로
  STANDBY 를 맞춘다. 영역 경로(`ServiceLogging.Dir` 등)도 같은 이유로 공통이다.
...
- **주입** — 두 번째 노드에 oam 을 설치하면 살아있는 OAM 의 레이아웃(사이트 디렉터리·마운트)이 배포설정에
  주입된다(`_materialize_deploy_config`, overlay 가 레이아웃을 정하지 않았을 때만 — 이관이 넣은 값을 되돌리지
  않기 위해). 그리고 실체화가 영역 경로를 **함께** 렌더한다(하나만 빠지면 반쪽 설정이다):

  | 키 | 출처 | 빠뜨리면 |
  |---|---|---|
  | `CimsSiteDir` | 입력(overlay 우선, 없으면 base 주입) | 단일 루트 레이아웃으로 유도 — 다른 노드와 영역 경로가 갈린다 |
  | `CimsRuntimeDir` | 유도 `<사이트>/runtime`(override 입력 가능) | 마운트에서 유도(`<마운트>/runtime`), 마운트도 없으면 노드 로컬 실효 루트 |
  ...
  | `Packages.Dir` | 유도 `<사이트>/packages`(단일 루트면 `<store>/pkg_files`) | 패키지 `oam.json` 의 상대경로로 폴백해 **버전 디렉터리**를 본다 → 그 노드가 Active 가 되는 순간 `/agent-bundle.tar.gz` 404 (agent·모듈 설치/업그레이드 전면 불가) |
...
- **여러 사이트가 NAS 하나를 나눠 쓰는 구성**(예 사이트 디렉터리 `/mnt/cims/test48`) — 경로 설정이
  정본이므로 마운트 하위 어느 경로든 그대로 사이트다. 명시한 store 경로(`CimsRuntimeDir`)도 마운트와
  상관없이 그대로 쓴다 — 마운트 하위가 아니면 mount guard 가 기동을 거부하므로 둘을 맞추는 것은 운영자의
  몫이고, 옛 값을 걷어내는 것은 이관(§9.4)의 일이다. `sweep_overlay_schema` 는 렌더가 달라지므로 그 키를
  지우지 않고 경고만 남긴다.

#### 경로 파생 규칙 — 사이트 디렉터리 영역

경로는 사이트 디렉터리 하나에서 영역별로 유도된다(정본 [site_directory_layout.md](site_directory_layout.md) §3,
구현 `services/paths.py`). 기준을 섞으면 이관 후 한쪽만 옛 위치에 남는다.

| 키 | 사이트 디렉터리 있음 | 없음 — 마운트만 | 없음 — 부트스트랩 직후(노드 로컬) |
|---|---|---|---|
| `CimsRuntimeDir` | `{사이트}/runtime` | `{마운트}/runtime` | `{노드 로컬 modules/oam/runtime}` |
| `Packages.Dir` | `{사이트}/packages` | `{CimsRuntimeDir}/pkg_files` | `{노드 로컬 store}/pkg_files` |
| `ServiceLogging.Dir` | `{사이트}/log` | 명시값(부트스트랩이 `{마운트}/service_log` 로 적는다) | `{노드 로컬 runtime}/service_log` |
| 녹취·콘텐츠·통계·상태 | `{사이트}/recordings` · `content` · `stats` · `state` | 로그 루트 아래(단일 루트) | 로그 루트 아래(단일 루트) |

영역 키는 전부 선언된 **override 입력**이고, 비우면 위 규칙으로 유도된다. 모듈 쪽에도 같은 규칙이 있어
(`services.paths.runtime_store_dir` → `file_store.runtime_root`) 설정이 비어도 같은 경로로 수렴한다.
단일 루트 레이아웃에서 `ServiceLogging.Dir` 의 기준은 **실제 마운트**이지 store 의 마운트가 아니다 — 마운트만
붙이고 관리 store 는 노드 로컬로 둔 구성(부트스트랩 `--mount` 만 지정)에서도 로그는 마운트를 따라간다.
...
- 그래서 **store 복사에서 `service_log` 는 제외**한다. 포함하면 (a) 마운트 파생인 로그가
  ...
  새 위치에 합친다(정지 창 무영향, 시간축 분할이라 나중에 합쳐도 안전). 사이트 디렉터리 구성은
  패키지·콘텐츠 영역을 store 와 같은 정지창에 복사하고, 로그·녹취·통계 영역을 기동 뒤 합친다(§9.4).
...
<shared>/{ha_operations,auth_codes,refresh_tokens,verify_runs}/
<shared>/.owner.json                     ← 소유권 리스
(패키지 저장소는 사이트의 packages/ 영역 — 단일 루트 레이아웃이면 <shared>/pkg_files/)
```
...
경로와 같다. 서버 인증서는 이 CA 로 서명한다(유효기간 2년, 잔여 60일에 엔진이 자동 재발급). CA 를 만들 수도
읽을 수도 없으면 self-signed 로 내려간다 — 인증서가 아예 없을 때만이고(있는 것을 자가서명으로 바꾸지 않는다)
기동을 막지 않는 것이 우선이라 브라우저 경고만 감수한다.
...
### 5.3 설정 리로드 — 기동 중 정해진 값은 파일이 되돌리지 못한다

agent 의 `update_config` 는 설정 파일을 쓴 뒤 모듈에 **SIGUSR1**(재기동이 아니라 "다시
읽어라")을 보낸다. 모듈이 들고 있는 설정에는 두 종류가 섞여 있다:

| 종류 | 예 | 리로드 시 |
|---|---|---|
| 파일에서 읽은 값 | 포트·DB 주소·로그 경로 | 갱신한다 |
| **기동 중 코드가 정한 값** | `_ConsoleStaticDir`(콘솔 dist 실경로) · `_mgmt_net` · **마운트 미비 시 강등한 `CimsRuntimeDir`** | **보존한다** |

두 번째를 지우면 파일에 없으므로 되살아나지 않는다. 실측 사고 둘:

- 설정을 저장하는 순간 `_ConsoleStaticDir` 이 비어 콘솔이 `console_not_bundled` 로 죽었다
  (파일은 멀쩡히 그 자리에 있었다).
- 공유 마운트가 없어 **로컬 store 로 강등**해 뜬 노드가, 설정 저장 한 번에 파일값(마운트
  경로)으로 되돌아갔다 — mount guard 가 막으려던 "없는 마운트 지점 하부에 store" 상태다.

그래서 리로드는 `services/config_reload.apply_reload` 로 통일한다: **파일 값으로
갈아끼우되 런타임 override(`runtime_set`/`runtime_unset`)와 내부 파생 키(`_` 접두)는
보존한다.** 그 값을 진짜로 바꾸려면 재기동해야 한다 — 기동 시 자원을 잡아 결정되는 값은
애초에 무중단 반영 대상이 아니다. oam·oam-svc·csc 세 모듈 모두 같은 규칙을 쓴다
(csc 는 oam services 를 마운트하지 않아 사본을 둔다).

**재기동 억제의 예외** — cold 모듈은 마스터가 아닌 노드에서 새로 켜지 않는다(§6). 다만
**이미 그 노드에서 돌고 있으면** 운영자의 start/restart 는 존중한다. 관리평면 자기보존
(§9.5)으로 비-마스터 노드에서 `oam` 이 살아 있는 상황에서, 재기동 요청이 억제되어
**깨진 프로세스를 되살릴 방법이 없었다**(실측). 억제의 목적은 "마스터 아닌 곳에서 새로
뜨는 것" 방지이지 도는 것을 못 고치게 만드는 게 아니다.

**관리평면 밖의 복구 통로** — 절체 홀드(래치) 해제의 정규 경로는 콘솔이 내리는
`ha_clear_holds` job 이다. 그런데 **양 노드가 모두 래치되면 그 콘솔이 뜨지 못한다**
(cold 모듈은 마스터에서만 기동 → 승격 가능 노드 없음 → VIP 주인 없음). 실측에서 두 노드가
순차로 재기동 예산을 소진해 전면 정지했고, 남은 수단이 파일 수동 삭제뿐이었다. 그래서
agent 에 `--clear-holds <service>` 를 둔다 — OAM 없이 노드에서 직접 해제한다.
...
```text
시스템 > 시스템/인프라 > (그룹 선택) > 패키지 설정 > oam > 사이트 디렉터리 > [이 경로로 이관]
...
| 5 | **사이트 디렉터리 구성**(base oam overlay 에 `CimsSiteDir`)이면 사이트 전체를 옮긴다 — 2 에서 `CimsSiteDir` 를 마운트 지점으로 바꾸고(로그 경로는 건드리지 않는다 — 사이트에서 유도된다), 3 의 job 에 영역 이동을 싣는다: 패키지·콘텐츠 영역은 `site_copies`(정지창 복사), 로그·녹취·통계 영역은 `site_merges`(기동 뒤 합류). 영역 경로를 따로 적은 영역은 옮기지 않는다([site_directory_layout.md](site_directory_layout.md) §5) |
...
3. 모듈 정지 → 4. 복사(_secrets·cert·service_log 제외) → 4-b. site_copies(패키지·콘텐츠 영역)
   → 5. config.json 기록 → 6. 모듈 기동
7. (기동 후, 백그라운드) site_merges(로그·녹취·통계 영역) — 단일 루트 구성이면 이관 전 노드 로컬
   service_log 를 새 로그 루트로 합류
```
...
(사이트 디렉터리 섹션의 안내 + `[이 경로로 이관]` 버튼). 그대로 두면 HA 편입은 되는데 데이터는
...
| | 언제 | 어떻게 | 결과 |
|---|---|---|---|
| **설치 시점** | 신규 설치 (마운트가 이미 있어도, 아직 없어도) | 부트스트랩이 사이트 디렉터리와 (필요하면) 공유 스토리지를 묻는다 (아래), 또는 `--site-dir`/`--mount*`/`--runtime-mount` | 사이트 영역 전부가 **처음부터** 그 하위. 이관 불필요 |
| **이관** | 단일 노드로 운영 중이거나, NAS 를 나중에 붙였다 | 콘솔 `[이 경로로 이관]`(위 표) | 정지 → 복사 → 기동 (콘솔 30초 단절) |

**설치 시점에 묻는 것은 사이트 디렉터리 하나다** — 공유 스토리지를 새로 붙여야 하면 이어서 서버의
export 경로와 이 노드에 붙일 위치를 묻는다. 파일시스템·옵션·영역 경로는 거기서 유도한다.

```text
[6/7] 사이트 디렉터리 (예: /mnt/cims/site01) [Enter=노드 로컬]:   /mnt/cims/site01
        공유 스토리지를 새로 붙일 원본 (예: nas.example:/export/cims) [Enter=이미 붙어 있음·로컬 디스크]:
        이 서버에 붙일 위치 [/mnt/cims]:                          ← 보통 Enter (사이트 디렉터리가 그 하위여야 한다)
        파일시스템 [nfs4] (nfs4/nfs/cifs/ext4/ext3/xfs/btrfs):    ← 유도값이 기본, 고를 수 있다
        → … 를 /mnt/cims 에 nfs4 로 붙이고 사이트를 /mnt/cims/site01 에 둡니다.
```
...
| `CimsSiteDir` | 입력값 (`--mount-src` 만 주면 `{붙일 위치}`) |
| `CimsRuntimeMount` | `{붙일 위치}` (mount guard) |
| 영역 경로 | `{사이트}/runtime` · `packages` · `content` · `recordings` · `log` · `stats` · `state` ([site_directory_layout.md](site_directory_layout.md) §3) |
...
## 9.4.3.1 순차 무장 — 개시 국면의 선착 경합

`일괄 시작`(또는 개별 모듈 start)은 **선행 멤버만 먼저 무장**하고, 그 노드가 **VIP 를 실제로
잡은 것을 확인한 뒤** 나머지를 무장한다.

### 왜 필요한가 — VRRP 의 우선순위 규칙이 꺼져 있다

VRRP 자체는 **priority 로 결정**하는 프로토콜이다(높으면 skew 가 짧아 먼저 MASTER 가 되고,
낮은 MASTER 는 preempt 로 밀려난다). 그런데 이 제품은 두 가지를 얹었다:

| 얹은 것 | 효과 |
|---|---|
| `nopreempt` | "빼앗기 금지" — **먼저 MASTER 가 된 노드가 계속 쥔다.** priority 는 같은 선출 창에서 경쟁할 때의 tiebreaker 로만 남는다. 없으면 구 Active 가 복구될 때마다 자동 스위치백이 일어나 단절이 한 번 더 생긴다(공유 store 단일 writer 라 더 나쁘다) |
| `track_script` (`interval 2, rise 2`) | 검진 2연속 성공(≈4초) 전에는 FAULT — **선거에 참가조차 못 한다** |

둘을 합치면 선거가 "priority 비교" 가 아니라 **"누가 먼저 검진을 통과하나"** 가 되고, 한 번
정해지면 `nopreempt` 가 못 바꾸게 한다. 선행이 검진에서 한 번만 밀리면(2초) 그대로 뒤집힌다.

**실측**: Signaling 그룹 `일괄 시작` 에서 02 가 13:26:43.744, 01 이 13:26:43.920 무장(0.18초 차)
→ 02 가 최소 경로(7.3초)로 MASTER → `nopreempt` 로 고정 → priority 90 노드가 Active,
priority 100 노드는 BACKUP. cold 모듈이라 csp 도 02 에서 떴다.

`state MASTER` 로 선행을 지정하는 방법은 쓸 수 없다 — keepalived 가 `nopreempt` 와 모순이라며
경고 후 `nopreempt` 를 무시한다.

### 동작

```text
개시 국면(= 무장 렌더인데 아무도 VIP 미보유) 판정
  → 선행 = prefer_first(개별 start 시 그 노드) > record running 멤버(정확히 1대일 때)
           > 지정 마스터(priority 최대)
  → 선행만 update_ha 큐잉. 나머지는 job 을 만들지 않는다(= keepalived 를 켜지 않는다)
     + 그룹 레코드에 `pending_arm {leaders, peers, deadline}`
  → sweep(2초 주기, `sweep_pending_arm`)
       · 선행이 VIP 보유 관측 → 나머지 무장, 보류 해제
       · 선행 아닌 노드가 VIP 보유 → 보류 무의미 → 해제(그 노드가 Active)
...
- **미개시 그룹은 대상이 아니다** — `enabled=false` 로 렌더돼 keepalived 를 켜지 않는다.
  그래서 `[적용]` 만으로는 VIP 가 붙지 않는다(붙이는 스위치는 `일괄 시작` = 무장).

### 두 번 죽어 있던 발동 조건 (회귀 방지)

이 로직은 코드에 있었지만 두 곳에서 조용히 스킵됐다. 둘 다 로그도 남지 않아 아무도 몰랐다.

| 결함 | 내용 |
|---|---|
| 서비스 키 오조회 | `services[group.name]` 로 무장 여부를 봤다 — 실제 키는 **`ha_service_key()` = `g<id>`** 라 항상 `None` → `armed=False` → 블록 통째로 스킵 |
| 선행 후보 모호 | `record running` 이 양쪽에 남아 있으면 후보가 2대가 되고 `len(firsts) < len(renders)` 가 거짓이 되어 스킵. 이전 실행이 있으면 **흔히 이 상태**다 → 후보가 정확히 1대가 아니면 지정 마스터로 좁힌다 |

`tests/test_put_config_sync.py::TestSequencedArming` 8건이 이 회귀를 막는다.

## 9.4.4 마운트는 서버 추가의 일부다

공유 마운트를 별도 작업으로 두면 운영자가 잊는다. 그러면 **모듈은 다 들어갔는데 마운트만 없는
노드**가 생기고, 그 노드는 공유 store 를 못 써 승격 부적격(`store:not_mounted`)이 된다 —
실측: 계획 절체가 원본을 내려놓은 **뒤에야** 그 사실이 드러나 관리평면이 약 1분 끊겼다
(30초 롤백이 원본을 되살렸다).

그래서 마운트를 **서버 추가 흐름 안**에 둔다.

```text
[+ 시스템 추가] ── 공유 스토리지 마운트를 함께 적용 ☑     ← SA·AS 만 (AA 는 서버를 안 만든다)
                    원본 [nas.example:/export/cims]   ← 이 설치가 이미 쓰는 마운트가 기본값
                    붙일 위치 [/mnt/cims]
                    파일시스템 [nfs4]
        │
        ├─ agent 레코드 `pending_mounts` 에 선언 저장 (재설치·재발급에도 따라온다)
        └─ 그룹 레코드 `mounts` 에도 같은 선언 (그룹 화면의 '선언 vs 멤버 적용' 대조 근거)
                    │
        enroll 성공 ─┴─→ `apply_mounts` job → agent → `cims-priv mount-add`
```

- **AA 는 이 모달에서 마운트를 묻지 않는다.** AA 는 그룹만 만들고 서버는 0대다(멤버는 이후
  `[+ 멤버 추가]`). 붙일 대상이 없는데 입력을 받으면 "등록 직후 붙습니다" 가 거짓이 된다.
  AA 의 마운트는 전적으로 `[+ 멤버 추가]` 단계가 묻는다(그 단계의 기본값은 이 설치가 이미
  쓰는 마운트).
- **`[+ 멤버 추가]` 도 그 자리에서 묻는다 — 조용히 상속하지 않는다.** 이 경로로 들어오는
  서버는 그룹 생성과 며칠 떨어질 수 있다(AA 는 이 경로가 **유일**하고, AS 도 3번째 멤버부터
  그렇다). 그룹 선언을 묵묵히 상속하면 운영자는 "이 서버는 마운트가 되는가" 를 알 방법이 없고,
  그래서 마운트 없는 노드가 조용히 생긴다 — 실측: Media(AA) 두 노드(.136·.139)가 그렇게 돼
  서비스 로그를 쓰지 못했다(`[+ 멤버 추가]` 가 마운트를 아예 묻지 않았다).
  → `[+ 멤버 추가]` 는 **선택 단계**(서버 이름 + 마운트 여부·원본·위치·fstype)를 띄운다.
  기본값은 **그룹 선언 → 없으면 이 설치가 이미 쓰는 마운트**로 채우고, 확인은 그 자리에서 받는다.
  설치 명령 화면에도 확정값을 되짚어 보여준다.
- **선언의 세 상태를 구분한다.** `agent.pending_mounts` 가:

  | 값 | 뜻 | enroll 동작 |
  |---|---|---|
  | `[{…}]` | 이 서버에 이 마운트 | 그대로 적용 |
  | `[]` | **마운트하지 않음(명시)** | 아무것도 하지 않음 — 그룹 선언으로 뒤집히지 않는다 |
  | 키 없음 | 미지정 | **소속 그룹의 `mounts`** 로 폴백 (구 레코드·API·블루프린트 경로) |

  체크를 끈 것이 상속으로 뒤집히면 운영자 의도와 반대로 마운트된다 — 그래서 빈 배열도 저장한다.
- **기본값은 새로 저장하지 않는다.** 이 설치가 이미 쓰는 cims-managed 마운트를 읽어 제안한다
  (oam 호스팅 노드 우선). 값의 정본은 각 노드 fstab 이고 agent 가 heartbeat 로 보고한다.
- **집행 경로는 서버별 [마운트 관리]와 동일**하다 — `apply_mounts` → `cims-priv mount-add` →
  fstab `# cims-managed`. 그래서 **콘솔 마운트 화면에 자동으로 `● mounted` 로 표시된다**
  (`collect_mounts` 가 그 태그만 읽는다). 직접 `mount` 를 쓰면 표시도 영속도 안 된다
  (실측: 손으로 붙인 노드는 fstab 에 흔적이 없어 콘솔에 아무것도 안 보였고 재부팅에 사라졌다).
- **마운트 실패가 enroll 을 막지 않는다.** NAS 가 안 붙어도 agent 는 등록돼야 그 노드를 콘솔에서
  고칠 수 있다. job 실패로 남기고 마운트 화면에 `○ unmounted` 로 보인다. (부트스트랩은 패키지
  전개 전이라 즉시 중단이 맞지만, 이미 등록된 노드는 성격이 다르다 — §9.4.)
- 선언은 **지우지 않는다** — 재설치 시 다시 적용돼야 하고 `mount-add` 는 멱등이다.
- 유효하지 않은 선언은 **저장 시점에** 걸러낸다(절대경로·`..` 불가·source 필수·fstype 화이트리스트
  = `cims-priv valid_fstype`). job 이 조용히 실패하는 것보다 낫다.

이것으로 "운영자가 안 눌러서" 는 사라진다. 다만 절체 preflight(대상 승격 자격 사전 확인)은
별개로 남는다 — NAS 가 아직 붙지 않은 상태로 노드를 추가한 직후 절체하는 경우가 있다.
