...
관제사에게 보이는 PTT 그룹 전부(`GET /provisioning/directory/groups` = **관리 범위 안(org_code) ∪ 내 소유 ∪ 관제 그룹 청취 범위(`ptt_listen`) ∪ 내 멤버 그룹**,
멤버가 아니어도) · 검색 · [새 그룹]/행 [편집]/[삭제] → 종전 `GroupEditWindow`(GMS XCAP PUT/DELETE). 행의 `canManage`(관리 범위 안 또는 내 소유 — 서버 GMS 게이트와
같은 판정)가 참인 행에만 [편집]/[삭제]가 보이고, 청취 범위·멤버로만 보이는 행은 "청취 범위"/"멤버" 표지의 보기 전용이다(조직 미지정 그룹이 `own` 범위에서
통째로 사라지지 않게). 머리 힌트 = "PTT 그룹 N개 · 관리 가능 M개". 관리 범위가 없으면 GMS 목록의 내 멤버 그룹만 보이고 내 소유만 편집한다. §4.1 주소록 [그룹] 탭의 생성·편집·삭제는 그대로 둔다(멤버 그룹의 빠른 경로).
