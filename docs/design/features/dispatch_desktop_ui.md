...
│ │ 🎧/🔊 ▬▬▬ 로스터 이탈    ││┃│ (●)반이중 ( )전이중 ☐긴급││ │ [청취] 1004● 1005●통화   │┃│ ┌1 2 3┐ 주소록|최근      ││
│ └────────────────────────┘│┃│ PTT 주소록 ─────────────││ └────────────────────────┘┃│ │4 5 6│ 교통상황실 02-120││
│ ┌ 상황실 (멤버 8)  대기 ────┐│┃│ 윤순경 …0008  [사설][☐] ││ ┌ 대기열 ─────────────────┐┃│ │7 8 9│ 이순경 1003      ││
│ │                   [PTT] ││┃│ 최순경 …0004  [사설][☐] ││ │ 🔔 010-9876 → 7000 00:12│┃│ │* 0 #│ 소방상황실 …     ││
│ └────────────────────────┘│┃│ 박경장 …2003  [사설][☐] ││ │    울림 1001,1004 [당겨받기]│┃│ └─────┘ 정경장 1006      ││
│ ┌ 교통1 (멤버 5) ──────────┐│┃└────────────────────────┘│ └────────────────────────┘┃└────────────────────────┘│
...
**오른쪽 위: 일반통화 발신 — 다이얼패드 + 주소록**
- 번호 필드(내선·번호·이름 자동완성, `sip:` URI 허용) · [발신]=`dial(acc_volte, target)` · [픽업 **]=`pickup(code)`.
- **다이얼패드** 3×4(통화 중에는 DTMF 로 동작 — 같은 패드) · **주소록**(세그먼트 [주소록|최근]): 행 = 이름·번호·종류 배지(내선/외부)·현재 상태(그룹원은
  BLF)·[통화]·[문자]. 외부망 번호는 접속서비스의 트렁크 라우팅에 맡긴다(앱은 그냥 `dial`). 최근 = 착신·발신·부재(재발신).
- 주소록 소스: 관제 그룹원(프로비저닍) + 조직 연락처(외부망 포함 — 로컬 CSV, CSC 연락처 API 는 §13).
...
windows/dispatch-desktop/
  ...
  ViewModels/
    ...
    CallOriginateViewModel      ③ 오른쪽 위 — 다이얼패드(DTMF 겸용) · 주소록/최근
