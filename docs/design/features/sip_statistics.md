...
이 결손은 **PTT 칸에만 머물지 않는다.** 전 서비스 합계(`totals.all`)는 PTT 세션을 분자에
넣으면서 분모(시도)에는 못 넣으므로 **100% 를 넘는 값**이 나온다 — 실측(2026-09-11 TB):
VoLTE 2세션 + PTT 1세션에서 `success_rate 150% · completion_rate 66.7% · drop_rate 33.3%`.
완료율·드롭률은 방향이 반대다(분모에만 PTT 가 있다) — **없던 끊김 33%** 가 생긴다.

규칙: **시도를 원천에 남기지 않는 서비스가 섞인 축에서는 그 비율을 계산하지 않고 비운다**
(`null`). 개수(`sessions`·`talked`)는 사실이므로 그대로 낸다. 왜 비었는지는 `rate_gap`
(예: `["ptt"]`)으로 함께 낸다 — 화면이 *자료 없음* 과 *집계 불가* 를 구분할 수 있게.
...
고쳐서 내지 않는 이유는 **올바른 분모를 만들 수 없기 때문**이다. PTT 시도가 원천에 아예
없으므로 어떤 보정도 추정이 된다. 거짓값 대신 빈칸을 낸다 — NER 분자를 시도로 묶어
`103%` 를 막는 것과 같은 원칙이다(§5.1). **Y6 이 해소되면 이 규칙은 저절로 꺼진다**
(`stats_rollup._NO_ATTEMPT_SVCS` 를 비우면 된다).
...
**PTT 시도 장부 레코드** (`ptt/attempts/YYYYMMDD.jsonl`, CSP 가 결말마다 1줄 — Y6, 구현 중):
...
| 호 (PTT) — 시도 | **시도 장부** *(Y6 — 구현 중)* | `{ServiceLogging.Dir}/ptt/attempts/YYYYMMDD.jsonl` (CSP 가 결말마다 1줄) |
...
| Y4 | `PttSessionEnd()` 에 종료 사유 인자 추가 — PTT 완료율의 유일한 결손. **Y6 과 함께 간다**: 시도 장부만 넣고 이것을 두면 성공률은 나오는데 완료율이 0% 로 보인다 | CSP | |
...
**PTT 가 지표를 못 내는 원인은 한 곳이다.** CSP 는 `PttSessionStart()` 로 세션 디렉터리를
만들 때 비로소 기록을 남기는데, `ProcessGroupCall` 은 그 호출 **전에 여덟 갈래로 반환한다**
(`csp/GroupCallService.cpp`) — 그룹 없음 · 청취 인가 실패(403) · 청취인데 활성 세션 없음(480) ·
비멤버(403) · 서비스 코덱 없음(488) · SRTP 협상 실패(488) · 긴급/임박 개시 미인가(403) ·
세션 시간창 이탈. 전부 흔적을 남기지 않으므로 **세션 기록만 세면 성공률이 항상 100%** 다.

소통률(`turn_count > 0`)과 참여율(`people` / `member_count`)은 세션 인덱스만으로 산출
가능하므로 Y6 과 무관하게 먼저 낸다 — 지금 내고 있는 것이 그것이다.

| | 내용 | 대상 |
|---|---|---|
| **Y6** | 실패한 PTT 그룹통화 시도를 기록 — `PttSessionStart()` 이전 실패 경로가 아무것도 남기지 않아 **성공률의 분모가 없다**. 해소 = **시도 장부**(§2.3·§3): CSP 가 `ProcessGroupCall` 의 모든 결말(성립·거절 8경로)마다 1줄 | CSP |
