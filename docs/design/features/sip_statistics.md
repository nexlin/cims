...
#### PTT 는 NER 을 따로 내지 않는다

NER 은 *상대 사정으로 안 붙은 것*(통화중·무응답·거절)을 망 책임에서 빼주는 지표다. 그런데
**PTT 의 실패는 대부분 우리 쪽 구성 문제다** — 비멤버·권한 없음(403)·코덱 불일치(488)·세션
시간창 이탈. VoLTE 규칙을 그대로 적용해 이것들을 "사용자 사정" 으로 빼면 **구성 결함을
가린다.** 그래서 PTT 는 빼주는 항목 없이 `NER = 성공률` 이다(분자가 성립뿐).

지표가 하나 없어지는 게 아니라 **전 서비스 합계(`all`)의 NER 이 비로소 계산된다** — PTT 가
NER 정의를 갖지 않으면 합계 축도 낼 수 없다.
...
| 비운다 | 이유 |
|---|---|
| `success_rate` · `talk_rate` · `ner` | 분자에 PTT 가 있고 분모(`attempts`)에는 없다 |
| `completion_rate` · `drop_rate` | 분모(`sessions`)에 PTT 가 있고 분자(`completed`)에는 없다 |

`join_rate` · `talk_rate_sessions` 는 분자·분모가 모두 전 서비스에서 오므로 그대로 낸다.

고쳐서 내지 않는 이유는 **올바른 분모를 만들 수 없기 때문**이다. 시도가 원천에 아예 없으면
어떤 보정도 추정이 된다. 거짓값 대신 빈칸을 낸다 — NER 분자를 시도로 묶어 `103%` 를 막는
것과 같은 원칙이다(§5.1).

**이 판정은 서비스 이름 목록이 아니라 데이터로 한다** — `attempts` 가 0 인데 `sessions` 나
`talked` 가 있으면 그 축은 분모를 원천에 남기지 않은 것이다(`rate_gap: ["no_attempts"]`).
...
| `status` | 결말 SIP 응답코드(403·488·480…) — 성립이면 비움 |
...
세션 시간창 이탈. 그 여덟 결말과 성립 1곳이 전부 **시도 장부**(§2.3)에 1줄씩 남는다.
