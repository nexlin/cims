...
**원문은 항상 유효한 UTF-8 로 적는다.** JSON 은 UTF-8 만 허용하므로(RFC 8259 §8.1),
문자열 값은 공용 이스케이퍼 `SimpleJson::JsonNode::Escape` 를 거친다 — 제어문자는
`\uXXXX`, **유효한 UTF-8 시퀀스가 아닌 바이트는 `\ufffd`(U+FFFD)** 로 바꾼다. 한글
표시이름 같은 정상 다바이트는 그대로 통과한다. SIP 포트로 들어오는 것이 전부 SIP 는
아니어서(인터넷에 열린 주소에는 스캐너의 DTLS/TLS 탐침이 온다) 임의 바이트가 원문에
실릴 수 있는데, 그대로 적으면 그 JSONL 이 UTF-8 이 아니게 되고 **파일을 읽는 통계
집계·이력이 통째로 멈춘다**. 읽는 쪽도 대칭으로 `errors='replace'` 로 읽어, 한 줄의
손상이 그 줄에서 끝나게 한다(단위시험 `tests/json_escape_test.cpp`).
