...
| UX 목업 `/test/mockups` | 시험 메뉴의 한 항목 — 아래 목업 다섯 개(토폴로지 캔버스·시나리오 캔버스·실행 라이브·결과 보고서·회귀 비교)를 탭으로 전환하는 허브(`TesterMockupsPage`). 탭 = 각 목업의 경로(사이드바 hidden, 직접 URL 가능). 정식 화면이 들어오면 해당 탭·목업 HTML 을 지운다 |
...
| 시나리오 캔버스 `/test/scenario-canvas` | **시나리오 편집 UX 의 목업**(검토용, API 호출·저장 없음 — 컨트롤러 `vocab`·`compile-check` API 뒤 정식 편집기로 `/test/scenarios` 에 들어온다). 기준 토폴로지 = 토폴로지 캔버스 목업의 프리셋 3종, 문서 = 동봉 시나리오 7종. 정식 편집기 사양 = ① **팔레트**(왼쪽 — 역할 UE/피어 · 단계를 축별로 접기: 등록/호/미디어/피어 축/전달·합류/PTT·MCData/이벤트·게이트, 워커 미지원 단계는 회색) ② **시퀀스 캔버스** — **레인 = 역할**(헤더: 이름·풀·kind 배지 ue/ibcf/pbx/mgcf·`disjoint_from` ⟷·기준 토폴로지 해석 줄 = 신원 수·워커·피어 고정 📌), **행 = 단계**(번호·종류·요약·레인 위 점/화살표(from→to, 라벨 = 코덱/코드/after_ms/Q.850)·`media_hold`/`wait` 는 전폭 띠·기대치 칩), **구간 띠 자동**(컴파일러 규칙 그대로 prelude = 앞쪽 register/wait · body · epilogue = 끝 deregister — body 안 register 는 붉게+오류), 행 사이 드롭 삽입·행/레인 드래그 재배열·바인딩 바(`${var}` 미리보기 값·프로파일 선택) ③ **속성 패널**(오른쪽 — 시나리오: id/title/tags/머리 주석/`target_evidence` · 역할: 이름(참조 동시 개명)/pool(토폴로지 풀 이름 ∪ group datalist)/`disjoint_from`(같은 풀만)/`count` + 해석 박스 · 단계: 종류별 폼 — 행위자 역할 칩은 단계의 kind 게이트로 비활성(progress/refer/cause = 피어, register = UE 또는 트렁크 계정 피어), after_ms, seconds 고정값/바인딩 토글, 코덱, payload, Q.850 목록, **기대치 편집기** = 단계에 맞는 지표 우선 + 전체, 임계 =/p50/p95/p99/max/min) ④ **하단 드로어** = YAML(양방향 — 캔버스 편집은 머리 주석 보존·한 줄 = 단계 재직렬화, YAML 편집은 [적용]) · 검증(스키마·행위자·구간·kind 게이트·바인딩, 항목 클릭 → 행/레인) · **토폴로지 적합성**(`compile_run` 드라이런: 역할→풀→워커 창 `[b,e)`, 피어 고정, 대상 시드 예정 컬렉션, 트렁크 환경변수, 프로파일 결합 시 **Little 검산** SApS × SDT ≤ 역할 신원) · 절차표(보고서와 같은 절차·예상 결과·확인 방법) ⑤ 툴바 = 시나리오/기준 토폴로지 선택 · 검증 배지 · 템플릿에서 새로 · 되돌리기 · 저장(동봉본이면 운영자본 override) · 단발 실행 · 프로파일 결합. **시나리오 모델 확장(목업에서 확정한 제안 — 컨트롤러 Step 모델·워커 이행 대상)**: ⓐ **세션 열** = 레인 밖 세로 레일, 다이얼로그마다 INVITE~SDP 교환 점선 + 당사자별 송출 가닥(초록 = 송출 중·샘플명, 빈 가닥 = 확립됐으나 송출 없음, 회색 = 시그널링만) — 행 왼쪽에 body 누적 시각 `t+`. ⓑ `media_hold` 정의 = **확립을 기다린 뒤 N 초 통화를 이어가고 끝에 RTP 표본**(ETSI TS 186 008 HT) — 미디어 시작/정지와 무관, 개명 후보 `talk`(SIP `hold` 와 충돌). ⓒ **in-dialog 단계**(dtmf/hold/resume/refer)는 확립된 세션 안에서만(컴파일·편집 검증). ⓓ **`media_hold.during[{at_s, step, …}]`** = 유지 구간 안 시각 지정 동작 — 컴파일러가 평평한 단계열로 푼다(`hold 3 → dtmf → hold 17` 등가, 판정은 마지막 조각 표본), 워커 계약 무변경. ⓔ **`invite.media.rtp = auto | none | explicit`** — auto = 183+SDP 또는 200 부터 양쪽 기본 샘플 자동(현 동작), none = SDP 오퍼하되 RTP 소켓 없음(시그널링 전용, SIPp 관례 — RTP 지표 판정 불가 경고, 워커 RTP 용량 미소모), explicit = `media_send` 가 켠다. ⓕ **`media_send{who, sample, after_ms, loop}` / `media_stop{who}`** — 허용 위치 = 그 역할이 SDP 를 주고받은 뒤(착신: `progress`(183+SDP)·`answer` 를 보낸 뒤, 발신: 받은 뒤 — RFC 3264/3960), m-line 당 활성 샘플 하나(교체 = 링백→통화음), `after_ms` 는 직전 SIP 이벤트 기준 지연, 수신은 항상 켜짐. ⓖ **샘플 라이브러리** = 토폴로지 `media.samples{id: {코덱: 파일 | synthetic}}` + 워커 `media{samples[](health 보고), rtp_ports, max_rtp_streams}` — 컴파일 검사가 run 워커마다 참조 샘플 보유·협상 코덱 인코딩을 확인하고 SIP 용량(`max_endpoints`)과 RTP 용량을 나눠 잰다. 워커 변경 = `CRtpThread` 샘플 교체·송신 일시정지 API, 확립·183 자동 Start 의 모드 분기, `hold` sendonly 시 상대 송신 정지(현재는 시그널링만 바뀜). 미디어 워커 분리(풀 `media_worker`, SIP 워커가 SDP 에 미디어 워커 주소 광고)는 워커 간 제어 채널이 필요해 후속. 채택하지 않는 것 = 시나리오에 토폴로지 id 저장(재사용성) · 분기/반복(워커 의미가 선형) · 토폴로지별 변종(풀 `group` 이 푼다) · 세션 상태의 레인 오버레이(레일로) |
| 실행 라이브 목업 `/test/run-live` | **실행 화면 재구성 UX 의 목업**(검토용, API 호출 없음 — 확정되면 `/test/runs` 가 이 사양으로 바뀐다). 사양 = ① **워커·대상 띠**(맨 위 — 워커마다 SIP 단말·RTP 스트림(§7 시나리오 ⓖ 워커 media 용량)·CPU 막대, 최대 SApS·진행 run·시계 오차, 미응답은 점선; 대상 카드 = 접속점·빌드·CPU·5xx·활성 알람(F 단계) + 마지막 연결 검사 결과) ② **라이브 패널** — 머리(판정·상태·id·시나리오·라벨·토폴로지/프로파일/대상 빌드·라이브 점) + **진행 막대**(경과 / 예상 소요 = 단계 수 × hold_s / 종료 예정 시각) + 왼쪽 **단계 사다리**(step 프로파일의 율 단계마다 완료 ✓·IHS %, 현재 단계 진행 막대, DOC 후보 표시, 신원 부족 예상 단계는 흐리게) + **종료 조건 게이지**(stop_on IHS·대상 CPU·CSP 5xx 각 현재/임계 + **Little 검산** 신원 여유) + **KPI 타일에 기대치 모서리**(≤ 2000 · ok/bad — 시나리오 expect 에서 유도) + **지표별 소형 차트 6장**(시도율·실패/초 · 동시 세션 · SER · SRD p95 · RTP 손실 · 워커/대상 CPU — 지표마다 자기 값 축 + 붉은 점선 기대치 + 단계 띠 공유 + 십자선 호버로 전 차트 값 동기; 한 축 겹침 차트를 대체한다) + **절차 진행 표**(시나리오 flow 단계별 진입/완료/진행/실패 + 누계 기대치 판정 칩 — 인스턴스가 어디에 몰려 있는지) + **실패 이벤트**(응답 코드 칩 필터·단계별 집계, 행 → **SIP 사다리 드로어**) + 워커 로그 접기 + 조작 = 율 적용 · **단계 고정**(프로파일 시계 정지, 율 유지) · 즉시 중단 ③ **run 색인** — 필터 바(라벨/id 검색·시나리오·대상 빌드·판정 칩·부하 run 만·기간), **날짜 그룹**, 열 = 판정 먼저·시각·시나리오/라벨(중단 사유 배지)·프로파일·대상 빌드·시도·SER·SRD p95·손실·소요, 행 호버 액션 = **재실행**(같은 조건으로 시작 창)·결과·더 보기, 체크 → 하단 고정 비교 바(첫 선택 = 기준) ④ **run 시작 창에 계획 미리보기** — 역할→풀→워커·신원 창 표, 예상 소요, 피크 율·동시 세션(SDT ≈ ht+2.5), 워커 SIP/RTP 용량, 대상 시드 예정 컬렉션, 바인딩 출처, **Little 검산 경고**(피크 SApS × SDT × UE 역할 수 > 신원 → skipped 예상 단계·권고 max), 워커 최대 SApS 초과 경고(워커 2대 분산), 알려진 CSP 과제(§12) 경고. **필요한 컨트롤러 API(제안)** = `POST /runs/plan`(compile_run 드라이런 — 시나리오 캔버스의 compile-check 와 같은 함수) · `POST /runs/{id}/hold`(단계 고정/재개) · `GET /runs/{id}/sip/{call_id}`(실패 호 덤프) · `GET /runs?scenario=&build=&verdict=&since=`(색인 필터) · 워커 health `media{rtp_streams,max_rtp_streams}` |
| 결과 보고서 목업 `/test/run-report` | **결과 화면 재구성 UX 의 목업**(검토용). 사양 = ① **왼쪽 run 레일**(날짜 그룹·판정 점·검색·FAIL/PASS/부하 칩, ↑↓ 이전/다음 — 툴바 400 px select 를 대체) ② **판정 요약 머리**(왼쪽 색 띠 = 판정, 큰 PASS/FAIL + 기대치 n/m, **왜 그 판정인가** 한 문장 = 실패 기대치마다 `#단계 종류 지표 기대/관측`·중단 사유·알려진 CSP 과제 §12 연결·PASS 면 DOC, 메타 칩 한 줄(run·토폴로지·프로파일·대상 빌드·시작/소요·워커·역할→풀·바인딩), 액션 = **이전 빌드와 비교**(같은 시나리오·프로파일의 직전 빌드 run 을 자동으로 기준에)·같은 조건 재실행·시나리오 열기) ③ 고정 **구획 내비**(판정·시간축·지표·절차·실패·단계 로그·대상 증거·Markdown) ④ **시간축** = 소형 차트 6장(실행 라이브와 같은 구성) + **대상 알람·이벤트 타임라인 레인**(run 창으로 잘라 세로 마커로 전 차트에 겹침 — F 단계) ⑤ **지연 분포 표 → 행 펼침 히스토그램**(로그 상한 버킷 1·2·5·…·60000 막대 + p50/p95/p99 마커 — 컨트롤러가 버킷을 내려 줘야 한다) ⑥ **절차·예상·확인 표에 여유 막대**(관측/임계 비율, 80 % 넘으면 주의색, FAIL 행 강조) ⑦ 실패 이벤트 = 코드 칩 집계 + 행 → SIP 사다리 드로어(전체 덤프·대상 CSP flow 로그 sesid 상관 링크) ⑧ 단계 로그 = IHS 막대(임계 50 % 위치 표시)·DOC·임계 초과→중단 배지 ⑨ **대상 증거 카드**(target_evidence 항목별 대기/OK — F 단계 대상 OAM 조회, seed_restored 는 지금도 판정 가능) ⑩ Markdown 미리보기·복사. 인쇄는 레일·내비·액션을 숨기고 구획 단위로 쪽 나눔. **필요한 API(제안)** = `GET /runs/{id}/hist?timer=`(버킷 분포 — metrics.sqlite 에 이미 있음) · `GET /runs/{id}/sip/{call_id}` · `GET /runs/{id}/target-alerts`(F 단계 — 대상 OAM `/alerts` 를 run 창으로) |
| 회귀 비교 목업 `/test/run-compare` | **비교 화면 재구성 UX 의 목업**(검토용). 두 모드 — **run 비교**: ① **run 카드 열**(색 견본 = 차트 색, 첫 카드 = 기준, 드래그로 순서 변경·[기준으로]·제거, [+ run 추가] 는 **같은 시나리오·프로파일 후보를 먼저**(다른 것은 흐리게, [전부 보기])) ② **지표 매트릭스**(행 = 지표·방향, 열 = run(대상 빌드로 표기), 셀 = 값 + Δ(pt 또는 %) + **중심 0 의 Δ 막대**, 회귀 붉게·개선 초록, 허용 오차 열 ±0.5 pt/±5 %) ③ **시간축 겹침**(매트릭스 행 클릭 → 그 지표의 run 별 시계열을 **run 시작 t+0 기준으로 정렬**해 한 차트에, 기준 run 굵게, 기대치 점선, 기준 run 의 단계 띠 — 프로파일이 다르면 경고) + 응답 코드 분해 run 별 ④ **기대치 판정 diff**(행 = 단계·지표·기대, 열 = run 별 PASS/FAIL, 변화 열 = PASS→FAIL **회귀** / FAIL→PASS **복구**, 하단 중단 사유) — **추세**: 시나리오 × 프로파일 × 토폴로지 하나의 모든 run 을 시간순 점으로(x = run 시각, 색 = 대상 빌드, ● PASS / ◆ FAIL·중단, 기대치 점선, **기준 = 첫 PASS run ± 허용 오차 회색 띠**, 띠를 벗어나면 '회귀' 라벨) 지표 4장(SRD p95·SER·손실·DOC) + run 목록(체크 → run 비교로). 내보내기 Markdown/CSV. 시나리오가 다르면 경고 배지(지금과 같음). **필요한 API(제안)** = `GET /runs/compare?ids=&format=md|csv` · `GET /runs/trend?scenario=&profile=&topology=`(색인 요약만이라 컨트롤러는 색인 필터로 충분) · 시계열은 기존 `/runs/{id}/series` |
...
**남은 것** = 위 목업 세 화면의 정식화(실행·결과·비교 재구성 + 제안 API `runs/plan`·`hold`·`hist`·`sip/{call_id}`·`target-alerts`·색인 필터·`compare?format=`)·대상 알람 타임라인 겹침(F 단계 대상 관측)·cims-verify S3/S6 시나리오 항목의 `cims-tester` 호출 이전. 목업 라우트 다섯 개(토폴로지·시나리오 캔버스, 실행·결과·비교)는 정식 화면이 들어오면 지운다.
