...
| **18x SDP 의 미디어 앵커링**(early media RTP) | **미구현 — 실측 확인**(`TRUNK-MGCF-EARLY-MEDIA`, CSP SIP 로그·CMP 로그·CMP 녹취 대조): ① CSP 가 B-leg 183 의 SDP 를 **재작성 없이** 발신자에 전달한다 — 발신자가 받는 183 의 `c=`/`m=` 이 피어 자신의 RTP 주소(200 의 SDP 는 CMP A측 주소로 재작성됨) ② CMP 에 B-leg 주소를 넘기는 `RELAY_MODIFY`(`setRemote peer[1]`)가 183 이 아니라 **200 에만** 나간다 | 피어가 CMP B측 포트로 보낸 링백 RTP 는 주소 미등록이라 200 까지 폐기(`early_rtp_pct` 0 % — 발신자 수신 0), 발신자의 early RTP 는 CMP 를 우회해 피어에 직접 간다(CMP 녹취·SRTP 종단·NAT latch·토폴로지 은닉 밖). 같은 호스트 시험망이라 직접 경로가 열려 있을 뿐, NAT·망 분리 환경에서는 early media 가 양방향 모두 끊긴다. 18x+SDP 에서도 200 과 같은 앵커링(SDP 재작성 + `RELAY_MODIFY`)이 필요하다(RFC 3960 early media, TS 24.628) |
