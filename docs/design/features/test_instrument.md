...
**추가할 것**: MCData 대용량 SDS/FD 의 MSRP 미디어 평면(C-plane SDS MESSAGE 는 구현 반영 ⑪), RFC 4028 세션 타이머 응답, 신원마다 개별 행동 스크립트(§4).
