...
컴포넌트는 팩 안 `components/`(MiniChart · LiveCharts · WorkerFleet · LiveSidebar · RunLivePanel · RunIndex · RunStartDialog · PlanPreview · SipDrawer · RunReport · YamlEditor · topology/TopologyCanvas · scenario/ScenarioCanvas), 모델·표시 헬퍼 `lib/`(fmt.ts = RFC 6076 라벨·판정 톤·수치 형식 · metrics.ts = 기대치 임계·라이브 누계 판정·절차 진행·SDT/예상 소요(컨트롤러 규칙과 동일) · topology-model.ts · scenario-model.ts). 차트는 라이브러리 없이 SVG(`--chart-N` 토큰), 지표별 소형 차트는 자기 축.
**남은 것** = 대상 관측(F — 알람 마커는 API 로 붙지만 대상 CPU·target_evidence 판정은 대상 OAM 조회 후속) · 워커 SIP 덤프 이전(`runs/<id>/sip/<call_id>.log` 규약만) · 시나리오 모델 확장 ⓔⓕⓖ 워커 이행 · cims-verify S3/S6 시나리오 항목의 `cims-tester` 호출 이전 · 워커 자동 발견(agent 배포 목록).
