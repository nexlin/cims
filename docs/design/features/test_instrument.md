...
**남은 것** = 대상 관측(F — 알람 마커는 API 로 붙지만 대상 CPU·target_evidence 판정은 대상 OAM 조회 후속) · cims-verify S3/S6 시나리오 항목의 `cims-tester` 호출 이전 · 워커 자동 발견(agent 배포 목록) · 미디어 평면 후속(AMR-WB 샘플 동봉·비디오 송출 제어·미디어 전담 워커 분리).
