...
([ue_nat_traversal.md](ue_nat_traversal.md))가 시험 대상. 동봉 `VOLTE-CALL-NAT`(volte_ue_nat). `invite`/`group_call` 의 `expect.code` 가 300 이상이면 그 최종 응답이 성공 조건이다(ACL 403·비멤버 403·라우팅 reject) — 워커는
