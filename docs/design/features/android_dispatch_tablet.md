...
| `PersonActionsViewModel`·`DeskViewModel` | **미구현** | 사람 메뉴·오늘 데스크(§11) |
...
- **사람 메뉴·통합 검색·오늘 데스크** — 데스크톱의 `PersonActionsViewModel`·`Ctrl+K`·`DeskViewModel` 에
  해당하는 것이 아직 없다(§6.8). 셋 다 **기존 화면의 진입점**이라 화면을 새로 만들지 않고 붙일 수 있다 —
  사람 메뉴는 카드 롱프레스, 검색은 상단 바, 오늘 데스크는 ③ 패널 줄.
