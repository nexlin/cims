...
| 화면 배치 | 최상위 넷은 **하단 내비**, [관제] 안은 **탭 2개**(PTT / 일반통화) | dispatch_desktop_ui.md §12 |
...
최상위 넷([관제]·[이력]·[PTT 그룹]·[관리], dispatch_desktop_ui.md §3.4)은 **하단 내비**다.
별창은 없고 같은 창의 레이어 전환이다. 관제 밖 화면 상단에는 **관제 요약 띠**(§3.5 — 발언 대상·발언 상태·
대기열·미읽음·내 통화·감청 수·[관제로])가 붙고, 세션을 만드는 조작은 관제로 자동 복귀한다.
...
**착신 카드만으로는 전화를 받을 수 없다.** 카드는 [관제] > [일반통화] 탭 안에만 있고 기본 탭은 [PTT] 라,
...
- 사람 메뉴: ③ 관제 그룹원 줄과 ⑥ 통화 내역 행의 **롱프레스**. 열린 메뉴는 «어느 줄에서 열렸는지» 를 키로
  하나만 둔다 — 줄마다 boolean 을 두면 스크롤 재사용 때 엉뚱한 줄에 붙는다.
...
### 6.3 관제 캔버스 — 6패널을 탭 2개로

가로 1280×800 기준. 상단 바 56 + 발언 바 64 + 하단 내비 56 = 176dp 를 빼고 **약 624dp** 가 본문이다.

```
┌────────────────────────────────────────────────────────────────┐ 56
│ 관제1석 · 대표 7000 ●● [감청 2▾] 🎧🔊 [검색]              14:32 ⚙ │
├────────────────────────────────────────────────────────────────┤ 64
│ [ PTT ]  발언 대상 3 · ●순찰1 승인 ●상황실 대기2 ●교통1 승인    │ ← 발언 바 (탭 바깥 고정)
├──────────────────────────────┬─────────────────────────────────┤
│  [PTT] 탭                     │  [일반통화] 탭                   │
│ ┌────────────┬─────────────┐ │ ┌──────────────┬──────────────┐ │ 340
│ │ ① 내 채널   │ ② 범위 채널  │ │ │ 그룹원 띠     │ 오늘 데스크   │ │
│ │            │             │ │ │ 대표번호 대기열│ 내 통화       │ │
│ ├────────────┼─────────────┤ │ ├──────────────┴──────────────┤ │ 284
│ │ ④ PTT 메시지│ ⑤ PTT 이벤트 │ │ │ ⑥ 일반통화 내역              │ │
│ └────────────┴─────────────┘ │ └─────────────────────────────┘ │
├──────────────────────────────┴─────────────────────────────────┤ 56
│  [관제]   [이력]   [PTT 그룹]   [관리]                           │
└────────────────────────────────────────────────────────────────┘
...
**발언 바를 탭 바깥으로 올린다.** 데스크톱에서는 ① 안에 있지만(1920 캔버스에서 ① 이 항상 보인다), 태블릿은
탭을 옮기면 ① 이 사라진다. 관제사는 전화를 받으면서도 무전한다 — 발언은 탭과 무관한 상시 조작이므로
**위치가 아니라 의도를 이식한다**. [일반통화] 탭에서도 같은 바로 발언·해제한다.

| 패널 | 태블릿 배치 | 데스크톱과 다른 점 |
|---|---|---|
| ① 내 채널 | [PTT] 좌상 | 발언 바가 빠져 카드 목록만. 카드 모드 **타일** |
| ② 범위 채널 | [PTT] 우상 | 그대로(필터·검색은 여기만) |
| ③ 일반통화 | [일반통화] 상단 — **2열**(좌: 그룹원 띠·대기열 / 우: 오늘 데스크·내 통화) | 데스크톱은 1열. 폭이 두 배라 세로 스크롤을 줄인다 |
| ④ PTT 메시지 | [PTT] 좌하 | 그대로 |
| ⑤ PTT 이벤트 | [PTT] 우하 | 그대로 |
| ⑥ 통화 내역 | [일반통화] 하단 전폭 | 그대로 |

**불변**: 카드 탭 = 포커스(④⑤ 가 따라간다), 카드 체크 = 발언 대상. 둘은 끝까지 섞지 않는다.

**대상 변경은 한 통로로만** — `applyTargets` 하나를 지난다(`toggleTarget`·`setSingleTarget`·
`pruneTargets` 전부). 대상을 바꾸면서 **빠진 호의 floor 를 놓지 않으면 그 채널이 계속 송출한다** —
화면은 새 대상을 가리키는데 마이크는 옛 채널로 간다. 실제로 `toggleTarget` 만 지키고
`setSingleTarget`(`Ctrl+n`·참여 자동 승격)이 빠져 있었다. 경로가 늘 때마다 같은 결함이 다시 생기므로
분산시키지 않는다. 불변: **대상에서 빠진 호는 반드시 `floorRelease` 를 받는다.**

**발언 소유권** — 해제는 «지금 대상» 이 아니라 **«요청해 둔 호»** 로 한다. 누른 채 대상이 바뀌면
(자동 승격·세션 종료·`Ctrl+n`) 원래 호에 `floorRelease` 가 가지 않아 **마이크가 열린 채 남는다**.
실제 마이크 차단은 코어 floor participant 가 release 로 수행하므로 앱이 놓치면 송출이 계속된다.
그래서 발언 대상 집합과 별도로 «요청한 호 집합» 을 들고, 대상 교체·전체 해제·제스처 취소·화면 폐기·
ViewModel 정리에서 모두 그 집합을 푼다. 해제는 멱등이다(취소와 폐기가 겹칠 수 있다).

**PTT 입력** — 누르고 있는 동안이 발언이다. `Button` 은 자체 클릭 처리가 길게 누르기와 겹쳐 뗌을 놓치므로
`Surface` + `pointerInput` 으로 직접 받고, 해제는 `finally` 로 보장한다 — 제스처가 취소되거나 입력
코루틴이 취소되면(화면 전환으로 Composable 이 사라지면) `tryAwaitRelease` 뒤가 실행되지 않는다.
...
감청은 **전면 바텀시트** 하나로 연다. 열린 감청·청취를 **카드로 나란히** 담는다 — 전환 조작(세그먼트·스와이프)을
두지 않는 것은 관제사가 «지금 몇 개를 듣고 있는지» 를 한눈에 봐야 하기 때문이다. 상단 바의 [감청 n] 칩이
어느 화면에서나 시트를 연다.
...
| `McDataMessagesViewModel`·`SmsMessagesViewModel` | `PttMessagesViewModel`(SDS) | SMS/LMS 는 서버 게이트웨이 과제(§11) |
...
| `MonitorWindowViewModel` | **없음** — `MonitorSheet` 가 세션을 직접 투영 | 창당 상태가 없다(시트는 목록 하나, §6.5) |
| `DispatchSummaryViewModel` | **없음** — `SummaryStrip` 이 다른 VM 을 투영 | 자기 상태가 없다(§6.10) |
...
| `DeskViewModel` | **없음** — 상단 바가 직접 그린다 | 데스크톱에서는 상단 바 VM(§3.2 — 신원·등록 점등·오디오 요약·시계·배치 프리셋)이다. 태블릿은 배치 프리셋이 없고 나머지는 상태를 가진 쪽(`MainViewModel`·`SettingsStore`·요약 띠)이 그대로 그려 VM 이 따로 필요 없다 |
...
관제 밖 화면 상단에 상시(dispatch_desktop_ui.md §3.5). **다른 VM 의 투영이라 자기 상태를 갖지 않는다** —
발언 대상·발언 상태·[PTT]·대표번호 대기열·문자 미읽음·내 통화·감청 수·서버 인증서 경고·[관제로] 한 줄.

- [PTT] 버튼은 발언 바와 **같은 컴포넌트·같은 색 규약**을 작은 크기로 쓴다(96×34dp). 발언 바와 똑같이
  누른 채 화면을 떠나면 해제한다 — 두 곳 모두 `DisposableEffect` 로 `releaseAll()` 을 보장한다(§6.3 F2 규칙).
- 서버 인증서 배지는 잔여 ≤ 30일에만 뜨고 닫기가 없다. 관제 화면의 배너가 없는 곳에서 **유일한 표면**이다
  ([sip_tls_signaling.md](sip_tls_signaling.md) §8.6.2).
- 감청 수는 상단 바 칩과 같은 집합(`SessionKind.isSheet`)을 센다.
...
- **SDS 보관의 사용자 격리** — 지금은 관제석 단위로 한 `messages.db` 를 쓴다. 교대로 사람이 바뀌는
  자리에서 앞 사람의 스레드가 보인다 — 로그인 주체별로 가르거나 로그아웃 때 비우는 정책을 정해야 한다.
- **사람 메뉴의 진입점 확대** — 지금은 ③ 그룹원 줄과 ⑥ 내역 행에만 붙였다(§6.2f). 데스크톱은 PTT 로스터
  칩·⑤ 활동 줄·주소록 행에서도 연다. 같은 `PersonMenu` 를 그 자리에 더 붙이면 된다.
- **조직 상위·소속 입력** — [관리]의 상위 조직·소속을 지금은 **코드 문자열**로 받는다. 조직 트리가 이미
  있으므로 콤보(평탄화 목록)로 바꾸는 것이 옳다 — 디자인 일괄 정리에서 함께 다룬다.
