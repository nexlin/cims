...
| **latch 갱신 규율** | 서버가 먼저 거는 요청의 목적지이므로 **상시 살아 있는 경로**여야 한다 → 갱신은 **UDP 소스로 한정**(REGISTER 도 예외 아님). 단말이 대형 요청을 TCP 승격하면 그 다이얼로그의 ACK/BYE·재-REGISTER 까지 TCP 로 오지만 그 연결은 유휴 타이머로 닫히고, 닫힌 뒤엔 NAT 뒤 단말에 서버가 TCP 를 새로 걸 수 없다. UDP 등록 플로우는 keepalive 로 상시 유지 | `csp/UserMap.cpp` `Insert` |
