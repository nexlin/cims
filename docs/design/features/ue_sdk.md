...
| PTT 그룹 생성·편집·삭제 (관제사 = authorized user) | GMS XCAP `PUT/DELETE /org.openmobilealliance.groups/users/{xui}/{group}` (문서 = GET 과 같은 OMA list-service + mcpttgi), 자격 `ptt.allowCreateGroup`, 목록 `is_owner`, `If-Match`/412 | `CscClient.getGroup/putGroup/deleteGroup` + `GroupDoc`(`toXml`/`parse` — pjlib 비의존 문자열 스캔) · `GroupSummary.isOwner` · `Profile.allowGroupCreation`. 변경 감지 = `subscribeXcapDiff("sip:gms_psi@…")` → `onMessage(xcap-diff)` | TS 24.481, RFC 4825, RFC 5875 |
