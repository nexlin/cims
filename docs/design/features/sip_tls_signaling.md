...
| 상황 | 현재 |
|---|---|
| **주소 변경** (NAT rebind·망 전환, transport 동일) | 규칙 ①([§4.3](#43-갱신-규칙--수신-transport-가-저장-transport-와-같을-때))로 추종된다 — TLS 등록 단말 포함 |
| **전환 순간** (UDP 엔트리 존재 + TLS REGISTER) | 갱신 차단 → latch 는 죽은 UDP 주소. 단말은 UDP 를 더 쓰지 않으므로 도달 전멸. 규칙 ②([§4.4](#44-전환-규칙--새-transport-가-tcp-가-아닐-때)) 미구현 |
| **자가복구** | 없음 — 가드에 막힌 REGISTER 도 수명·Contact 갱신은 수행(`UserMap.cpp` REGISTER 분기)하므로 만료 sweep 에 걸리지 않는다. `SendOptionsPeriod` 기본 0 이라 능동 감지도 없다([§4.5](#45-자가복구--판단-보류)) |

전환이 막힌 상태에서 운영자에게 보이는 모습은 "등록 실패"가 아니다 — 단말은 200 OK 를 받아
등록됨으로 표시되고 **발신도 된다**. 착신·통지가 안 되고 통화가 주기적으로 끊긴다.
...
"불일치 transport REGISTER 는 수명을 갱신하지 않는다"(→ 만료 sweep 이 엔트리를 회수하고 다음
등록이 삽입 경로로 재수립)는 [§4.2](#42-stream-등록-단말에서-깨지는-지점) 의 영구 stale 을 끊는
안전망이지만, **단독 적용은 위험하다.** 승격 TCP 로 재-REGISTER 가 반복되는 구간에서 수명 갱신을
거부하면 **살아있는 단말이 등록 만료로 삭제**될 수 있다. 등록 만료(통상 3600초)와 승격 TCP 유휴
사망(서버 600초) 중 어느 쪽이 먼저인지는 측정되지 않았다 — [§9](#9-구현-계획) 2단계에서 실측 후
결정한다.
...
현재 `CUserMap` 은 AoR 당 단일 슬롯이며 소비자가 광범위하므로([§4.1](#41-갱신-경로는-정확히-두-곳))
별도 설계가 선행되어야 한다.
