...
| **콘솔 상단 배너** | 구현 | `CertExpiryBanner`(공통 셸, `ReadOnlyBanner` 옆·breadcrumb 위) — 셸이 이미 구독하는 알람 스토어(`useAlarms`)에서 활성 `cert_expiring`(A-PRC-009) 알람을 골라 모든 라우트 상단에 상시 표시: mo 표시명 · 축(mo 가 `…/agent/cert/<module>/renew` 면 "자동 갱신 실패", 그 외 "만료 임박") · 잔여 일수(`threshold_info.observed`), 3건 초과는 "외 N건". 활성 알람 중 최고 severity 가 major 이상이면 위험 톤(`bg-destructive`), 아니면 경고 톤(`bg-warning-on`). 닫기 없음, 알람 close 로만 소멸(정상 상태는 렌더 없음 — 레이아웃 무영향). 별도 폴링·엔드포인트 없음. 셸 상태 띠 규약은 [console_design_system.md §4](../console_design_system.md) |
| **관제조작반 경고** | 구현(Windows) | 관제사는 매일 앉아 있는 사람이라 폐쇄망에서 가장 확실한 채널이다. SDK 가 마지막 성공 핸드셰이크의 peer 인증서 `notAfter` 를 관측한다 — SIP TLS 는 pjsua2 `onTransportState`(`Engine::tlsPeerExpiry()`), HTTPS 는 OpenSSL 전송의 peer 인증서(`CscClient::tlsPeerExpiry()`) → C API `cimsue_engine_tls_peer_expiry`/`cimsue_csc_tls_peer_expiry`(`cimsue_tls_peer_expiry_t`) → .NET `Engine.TlsPeerExpiry`/`CscClient.TlsPeerExpiry`. 관제조작반은 둘 중 짧은 잔여가 ≤ 30일이면 **배너 레이어**(관제 캔버스 포함 어느 화면에서나, [dispatch_desktop_ui.md §3.2](dispatch_desktop_ui.md))에 "서버 인증서 N일 후 만료 · `<host:port>` · `<subject>` · 만료일 · 자동 갱신 실패 신호 — 운영자에게 알리세요 (콘솔 알람 A-PRC-009)" 를 상시 표시(닫기 없음, ≤ 7일·만료는 진한 빨강 = 서버 critical 과 같은 단계) + 관제 요약 띠([§3.5](dispatch_desktop_ui.md)) 축약 배지(별창 포함). 로그인·TLS 등록 성공·1분 주기로 재평가하고 서버가 갱신되어 임계를 벗어나면 내린다 |
| **Android 앱 설정 화면** | 구현 | PTT 앱 설정 탭 "기타" 와 VoLTE 앱 설정 "서버" 카테고리의 **서버 인증서** 행. Android 앱은 아직 `libcimsue` 이행 전(pjsua2 직접)이라 SDK 코어와 **같은 규칙을 Kotlin 에서** 관측한다(`core`): SIP TLS 는 `CimsEndpoint.onTransportState`(CONNECTED·remoteCertInfo.validityEnd), CSC HTTPS 는 `CimsTls.apply` 가 붙이는 OkHttp 인터셉터(응답 handshake 의 peer leaf) → `TlsPeerObserver`(프로세스 단일, StateFlow) → `TlsPeerExpiry`(임계 30/7 상수·`worst` = 둘 중 먼저 만료). 표시 = 잔여 >30일 "N일 남음"(보조색) · ≤30일 "N일 후 만료"(호박) · ≤7일·오늘·만료(빨강) + 부제 `<host:port> · 만료일 · 자동 갱신 실패 신호 — 운영자에게 알리세요(콘솔 알람 A-PRC-009)`. 관측이 없으면(평문 접속) `—`. 로그아웃(libDestroy)에 SIP 관측을 지운다. 읽기 전용 — 단말이 고칠 수 있는 게 아니다 |
...
| 2 | **루트 키 오프라인 이관** — `/home/cims/certs/cims-service-ca.key` 를 매체 두 벌로 옮기고 온라인 사본을 삭제한다(사이트 CA 교차 서명은 그 매체에서만). 사내 .45 는 §8.6.4 로 체인 전환이 끝났고(CSC 4430·CSP 15061 = 루트→그룹 CA→leaf 2장, 단말 무변경), 다른 노드(.49 등 루트 직서명 leaf)는 같은 절차를 그 노드의 그룹 CA 로 반복한다 — 루트 키가 오프라인이면 그룹 CA 인증서를 매체로 가져가 서명해 온다 | 운영 | 온라인 노드에 루트 키 부재 · 전환한 노드 leaf 체인 2장 · A-PRC-009 `cert/<module>/renew` 미발화 |
