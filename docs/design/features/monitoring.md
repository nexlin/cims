...
|------|------|:------------:|:------------:|------|
| **1분** | `1m` | 60점/시간 | 최근 1시간 | 순간 부하·장애 시점 특정 |
| **5분** | `5m` | 12점/시간 | 최근 1시간 | 실시간 모니터링, 장애 탐지 |
| **10분** | `10m` | 6점/시간 | 최근 3시간 | 단기 추이 확인 |
| **1시간** | `1h` | 24점/일 | 당일 | 일간 패턴 분석 |
| **1일** | `1d` | 28~31점/월 | 최근 30일 | 주간 추이 |
| **1주** | `1w` | 4~5점/월 | 최근 3개월 | 주간 비교 (월요일 시작) |
...
**저장 계층은 1분·1시간·1일·월 넷이고, 전부 파일이다** (DB 테이블 없음). 5분·10분·주·년은
저장하지 않고 조회할 때 합산한다.
...
접어 만든 1시간 집계는 90일까지, 일 집계는 2년까지, 월 집계는 영구히 남는다.

년을 저장하지 않는 이유는 **저장된 거친 행이 완전한지 알 방법이 없기 때문**이다. 그 해의 월이
아직 다 만들어지지 않은 상태에서 쓰인 년 행이 그대로 정답 행세를 한다(실측: 년 2,705 / 실제
5,216). 월 단위로 읽으면 없는 달이 날짜 경로로 내려가고, 거기서도 없으면 응답의
`missing_days` 에 잡힌다 — 조용히 작은 값이 나오지 않는다.

#### 집계 흐름

```
원본(호 이력 call.json/session.json · SIP 원문 *.msg.jsonl)
      │ 1분 주기 집계 — 원본을 읽는 유일한 단계
      ▼
   [1m] 저장 ─┬─→ [5m] [10m]  합산
             ├─→ [1h] 저장
             └─→ [1d] 저장 ─┬─→ [1w] 합산
...
진행 중인 분을 집계하면 그 분이 끝나기 전에 값이 확정되어 뒤에 도착한 메시지가 누락된다.
대상 구간은 항상 `[watermark+1분, 지금-1분]` 이다. 집계가 없는 구간(도입 전·롤업 정지)은
조회 시 원본을 그 자리에서 **같은 함수로** 접어 답하고, 응답 `source` 가 `rollup`/`scan` 을
알린다.

> 상세(세는 단위 3계층·미결 호 되짚기·조회 계층 선택)는
> [sip_statistics.md](sip_statistics.md) 가 정본이다.
