...
- **진행중 구역 고정** — chat 그룹처럼 종료가 없는 세션은 시각 정렬에서 늘 맨 아래로
  ...
  진행중은 **조회 날짜와 무관**하게 전량이다 — 어제 시작해 아직 열린 세션도 오늘 보기의
  진행중에 있다(그 안에서 일어난 오늘 통화가 그 세션이라, 시작일로 걸러내면 어디에도 보이지 않는다).
...
발언권 타임라인  14:20:03 ~ 14:31:47 · 최대 동시 발언 3명    ‹ › − 4× + [전체]
  ...
  테스트001      ██████▌      ▐███████             ← 막대 클릭 = 그 화자 단독 재생 (영상 발화면 오른쪽 아래 미니 영상 도크 360×202 에 영상으로)
  테스트003           ▐████████▌
  테스트005               ▐███▌      ▐██▌
