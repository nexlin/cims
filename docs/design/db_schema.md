...
| | `volte_subscriptions.forward_busy_id`·`forward_no_reply_id`·`forward_no_reply_sec`·`forward_not_logged_in_id` / `voip_subscriptions.…` | migrate_subscription_cdiv.sql | 조건부 착신전환 CFB/CFNR(+시한 초, 0=CSP 기본)/CFNL 대상 번호(TS 24.604 — volte_supplementary_services.md §6A.4). '' = 전환 없음. CSC 가 쓰고 CSP 가 읽는다 |
