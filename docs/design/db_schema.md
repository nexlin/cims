...
| **관제 그룹** | `dispatch_groups` | cims_schema.sql + migrate_dispatch_groups.sql | 관제 그룹(픽업 그룹+대표번호+감청 범위, [dispatch_center.md](features/dispatch_center.md) §3·§8.1) — `id`(VARCHAR(64) 불변 키 `dg-xxxxxxxx`), name, `pilot_id`(UNIQUE, 대표번호), `service_ref`, `alert_mode`(parallel/sequential), `no_answer_sec`, `busy_members`(skip/alert), `overflow_target`, `monitor_scope`(none/own/listed/all), `ptt_listen`(none/listed/all), `listen_visibility`(hidden/visible), `directory_admin`(none/own/all — 관제 앱 조직/구성원/번호·PTT 그룹 관리 범위, migrate_dispatch_directory_admin.sql, dispatch_center.md §3.4), `org_id`(FK organizations SET NULL — `directory_admin=own` 의 루트) |
