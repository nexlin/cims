...
```
CCmpClient
  ├─ KeepAliveLoop (3초 주기 — endpoint 별 HEARTBEAT liveness + 주기적 STATS 포화 점검)
...
`KeepAliveLoop` 는 3초마다 각 endpoint 에 개별 HEARTBEAT 를 보내고, N cycle(≈15초)마다 STATS 로 포화를 본다
(HEARTBEAT 는 순수 OK echo 라 "응답하지만 포화된" 노드를 못 잡으므로 STATS `rtp_ports_free`+`ptt_rtp_ports_free`
합계로 판정). 판정에 따라 ring 을 갱신한다:
...
- **SATURATED** (STATS 가용 RTP 포트 0) → ring 제외(신규 세션만; 기존 세션은 유지, teardown 안 함).
...
각 endpoint h ∈ m_endpoints (EndpointHealth: iFailCount/bLive/bSaturated)
  ...
  └─ STATS 가용포트 0      → h.bSaturated=true, ring MarkUnhealthy(신규만 제외)
