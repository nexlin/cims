...
| 8 | 시안 AppBar 의 **접속 환경 칩** `[운영]` — 무엇이 환경인지 시안에 없다 | 콘솔에 "운영/개발" 개념이 따로 없다. `VITE_CONSOLE_TARGET`(dev/prod)은 다른 축 | **개발자 모드 스위치가 곧 환경 축이다**(사용자 확정) — 켜짐=`개발` / 꺼짐=`운영`. `utils/devMode` 의 브라우저 로컬 값이라 서버 속성이 아니라 "지금 내가 보는 모드"다. 우측에 따로 있던 「개발자 모드」 배지를 이 칩이 대신한다 | 사용자 확정 |
