...
`index.css` 는 **토큰 + 위젯 격자 CSS 만 남기는 선까지** 줄인다(1,502 → 812줄, §8.4).
통째 폐기는 위젯 편집 3파일을 손댈 수 있게 된 뒤의 과제다 — 남은 것이 그 파일들이 쓰는
`.panel`·`.toolbar`·`.scroll-fill`·`.empty` 와 2D 그리드 CSS, 그리고 `:root` 토큰이다.
