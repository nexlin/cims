...
- **탭 카운트의 「패키지 설정」 숫자** — 시안 `Sec/Tabs (신규)`(458:9160)는 카운트를 `hasCount`
  로 켜고, G1 그림(458:8626)은 `패키지 설치 3 · 패키지 설정 9 · 패키지 제어 3` 이다. 설치·제어는
  **모듈 수**라 바로 붙였다. 설정의 `9` 는 그 스코프의 **설정 필드 수**인데(`screens/INDEX.md`:
  서버 4 · SA 13 · 그룹 공통 9) 스코프별 필드 분리(S3·SA3·G3)가 아직이라 그 단계에서 붙인다.
  그때까지 설정 탭에는 카운트를 그리지 않는다 — 틀린 수를 띄우지 않기 위한 것이다
