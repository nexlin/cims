...
- **회선 카드**(종류별) — 보기 = 종류에 맞는 항목만. [편집] = 접속서비스·IMSI·비밀번호(변경 시)·인증(Digest/AKA + K/OPc)·채널 정책(유선 = TLS 고정, AKA = TLS 강제) +
  전화 회선은 **착신전환 서비스**(전환 번호 하나 + 조건 선택 — CFU 단독 / CFB·CFNR(시한)·CFNL·CFNRc 다중, `조건별 번호 따로` 펼침 — 응답에 컬럼이 없으면 조건부 항목이 숨는다)·**착신 거부**(DND)·**링백**(음원 라이브러리 select, WAV 업로드는 서비스 › 안내음성) +
...
**시각 계약 정본은 [console_design_system.md](console_design_system.md) 다** — 토큰 이름·값,
컴포넌트 계약, 절대 규칙은 거기가 정한다. 여기서는 **위젯 플랫폼이 색을 다룰 때의 규칙**만 적는다.

테마는 `index.css` 의 **디자인 토큰**(`:root` / `:root[data-theme="dark"]`)으로만 갈린다.
화면 코드는 색 리터럴을 쓰지 않고, **Tailwind 토큰 유틸리티**로 적는다 —
`bg-card` `text-muted-foreground` `border-border` `bg-brandsoft` `text-success` 처럼.
`var(--…)` 를 직접 쓰는 것은 유틸리티가 없는 자리(런타임에 색이 정해지는 차트·다이어그램)뿐이다.

| 쓰임 | 유틸리티 (토큰) |
|---|---|
| 카드·패널 바탕 | `bg-card` / 눌린 바탕 `bg-secondary` / 페이지 바탕 `bg-background`·`bg-muted` |
| 선 | `border-border` · 강한 선 `border-border-strong` · 마우스오버 `hover:bg-accent` |
| 글자 | `text-foreground` / 보조 `text-muted-foreground` / 비활성 `text-text-disabled` |
| 상태 | `text-primary` `text-success` `text-warning` `text-destructive` `text-info` + soft 배경 `bg-brandsoft` `bg-success-soft` `bg-warning-soft` `bg-dangersoft` `bg-info-soft` (짝 글자색은 `text-*-on`) |
| 차트 계열 | `--chart-1` ~ `--chart-12` · `--chart-muted` — **런타임 배정이라 여기만 `var(--…)`** 를 쓴다 |

**계열 색에는 상태색을 쓰지 않는다** — 계열은 "무엇인가"이지 "좋다/나쁘다"가 아니라서, 섞으면
초록 막대가 정상을 뜻하는 것처럼 읽힌다. 위젯 차트는 앞 5색을 순환하고(`seriesColor`),
발언자 색처럼 계열이 많은 목록이 6~12 를 쓴다. `--chart-muted` 는 '미분류'처럼 값이 아니라
빈자리를 뜻하는 계열용이다.

**다크를 깨뜨리는 네 가지 — 전부 실제로 났다**

- **없는 토큰 + 라이트 폴백**(`var(--muted, #6b7280)`) — 토큰이 없으니 폴백이 **항상** 적용돼
  다크에서 흰 배경·회색 글자가 그대로 남는다. 토큰이 **정의돼 있어도** 마찬가지다 — 그 폴백은
  죽은 코드이고, 이름이 바뀌는 날 라이트 색을 조용히 되살린다. **폴백은 적지 않는다.**
- **정의가 없는 클래스에 기대기** — 이름만 있고 CSS 규칙이 없으면 아무 효과가 없는데 있는 줄
  알고 쓰게 된다. Tailwind 유틸리티와 이름이 겹치면 오히려 엉뚱한 스타일이 붙는다.
- **파스텔 배경을 직접 적기** — 라이트용 색이 다크에서 흰 알약으로 뜬다. `*-soft` 토큰으로 적는다.
- **배경을 안 적은 폼 컨트롤** — 배경을 비워 두면 **브라우저 기본 색**이 나오고, `color-scheme`
  이 다크에서 그 기본을 회색으로 바꿔 얹은 글자와 대비가 무너진다. 배경과 글자색을 함께 명시한다.

**흰 글자를 받는 배경은 테마와 무관하게 어두운 쪽으로 고정한다.** 상태 토큰
(`--cims-success` 등)은 **전경용**이라 다크에서 밝아진다 — 배경으로 쓰면 흰 글자와의 대비가
오히려 무너진다(다크 `#22c55e` 위 흰 글자 = 2.3:1). Solid 배지는 `*-on-soft` 계열의 어두운
값을 쓰고, 그 위 글자는 `--cims-on-solid`(양 테마 `#ffffff`)다.

**색 리터럴을 남기는 예외 세 갈래**는 console_design_system.md §9 가 정한다 — 시퀀스
다이어그램 팔레트 · 인쇄 전용 컴포넌트와 `@media print` · 영상 무대 `#000`.
