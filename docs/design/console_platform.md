...
카드 안 배치를 가진 화면: 성능 통계 3개(`cims.stats.*`) · 알람·이벤트 이력 · 유형별 분석 ·
서비스 정의 · 내 대시보드 구성 · 비정상 세션 이력. 블록은 모두 레지스트리에 남아 있어 다른 화면에 낱개로 얹을 수도 있다.
...
| 떼어내는 것 | 이유 | 예 |
|---|---|---|
| 서로 다른 축의 독립 지표 | 하나만 봐도 말이 된다 | 가입자 / 번호 / 활성 호 / 그룹 / RTP 점유, 비정상 세션(탐지 수 / 치명 / 스캐너 종류 / 발신 IP 수) |
...
- 분해한 지표 카드는 **선언 표 하나(정본) + 팩토리**로 생성한다(지표별 컴포넌트 금지):
  ...
  `utils/alarmLabels.ts` 의 `SEVERITY_ORDER`/`SEVERITY_LABEL`.
...
- `/alerts/history` = `core.alarm-event-history` **하나**. 전환 탭 · 기간 선택 · 이력 표는 함께 조작하는
  한 벌(고른 탭과 기간이 곧 표의 의미)이라 위젯 안에 둔다.
- `/alerts/analysis` = `core.alarm-event-analysis` **하나**. 알람 블록 5(요약 타일·심각도 분포·일별
  발생량·코드별 표·유형별 표)와 이벤트 블록 4는 같은 기간 창을 여러 각도에서 보는 한 벌이라 함께 둔다.
...
인터페이스 카드 안의 블록 구성은 `service/console/src/widgets/statsScreens.tsx` 의 `CardRow[]`
선언이 정본이고(누수 회수는 페이지 컴포넌트 하나), 블록 자체는
...
- 누수 회수(`/stats/leak-reclaims`) — 조회 조건(날짜) + 지표 카드 4(총 회수·무RTP·RTP후 미해제·노드)
  + 회수 세션 목록을 한 페이지 컴포넌트가 그린다(`LeakReclaimsPage`). 블록은 위젯으로도 등록돼
  있어(`LEAK_RECLAIM_WIDGETS`) 다른 화면에 낱개로 얹을 수 있다. 화면의 뜻(0건이 정상)은 자리를 차지하지 않게 툴바의 `ⓘ`(`components/InfoDot`
