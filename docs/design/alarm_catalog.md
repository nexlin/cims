...
| type | 정의 | 대표 조건 |
|---|---|---|
| `security_violation` | **보안 이상 징후의 급증**(인증 실패 폭주·사기 호/스캐너·비인가 트래픽·채널 정책 위반) — 율 임계 기반 open/close. 보안 알람은 X.733 계열과 통지 성격(대응 주체·민감도)이 달라 `threshold_crossed` 로 흡수하지 않는다 | 채널 정책 위반 반복(A-SEC-003)·sec-agree 협상 거절 반복(A-SEC-004) — CSP 자기보고 구현, 인증 실패 급증(A-SEC-001)·fraud/스캐너 탐지(A-SEC-002 — 미구현) |
...
- 보안 축: `security_violation`(X.736 Security Alarm — DOMAIN `SEC`, 정의 코드 `A-SEC-NNN`,
  표준화 §3.4(a))은 A-SEC-001~004 가 속한 클래스다(§4). 구현은 A-SEC-003(채널 정책
  위반 반복)·A-SEC-004(sec-agree 협상 거절 반복) — 둘 다 CSP L2 자기보고의 같은 소스 계수기
  ([자기보고 §4](alarm_self_reporting.md))이며, A-SEC-001/002 는 구현 채택 시 같은 경로로 편입한다.
...
capacity_threshold — 신규 INVITE CPS / threshold_crossed — SIP 수신 이상) + 이벤트 6종
(process_started/stopping · call_monitored · emergency_mode_changed · affiliation_changed ·
regroup_changed). fm_catalog.json 선언과 구현이 일치한다.

**PTT 감사·상태 이벤트 3종**(§11.2 (b)) — 셋 다 그 동작을 이미 하던 자리에 `SendEvent` 를
얹은 것이고, 공통 이유는 **기존 흔적이 기간 조회로 닿지 않았다**는 데 있다.
- `E-STC-007` emergency_mode_changed — `CGroupCallService::ApplyInCallCondition` 의 상향/하향
  두 분기(`EmitEmergencyModeEvent`). 전이는 원래도 `PttLogEvent` 로 세션 이력에 남았지만
  **세션을 열어야 보였다.** 등급은 상향=새 등급, 취소=직전 등급(`iCur`) 기준이라
  임박위험 취소가 "긴급"으로 표기되지 않는다.
- `E-AUD-009` affiliation_changed — `CscfModule.cpp _EmitAffiliationChanged`. SUBSCRIBE·PUBLISH
  의 affiliate/de-affiliate 4지점 + **REGISTER 해제 경로**. 해제 경로는
...
- `E-AUD-010` regroup_changed — 생성은 `ModuleDispatcher` 의 ad-hoc 그룹 `Insert` 직후,
  ...
  **존재 사실 자체가 남지 않던** 구간이다. **1:1 사설콜(`priv-`)은 대상이 아니다** —
  CSP 가 사설콜에도 같은 `_isAdhoc` 플래그를 쓰지만 그것은 수명 표시일 뿐이고, 정의가 말하는
  것은 ad-hoc 그룹·regroup 이다(규격상 셋은 서로 다르다). 포함하면 1:1 통화 1건마다 감사가
  2건씩 쌓인다. regroup(그룹 재편성)은 기능 자체가 없어 발화 지점도 없다.

`E-AUD-009` 는 등록 빈도를 따라가므로 이벤트 보존 정책을 함께 본다(`E-AUD-006` 과 같은 성질). SIP 통계 축은 psip 스택 카운터(`CSipStackCounter` — 수신 요청·
...
숫자는 CSV(`행`·`기능`·`instance`·`현재 구현 여부` 열) 파생이다. CSV 를 고치면 함께 갱신한다.

**기능 카테고리 × 감지 모듈** (감지 행 185건, 셀 = `구현/후보` 또는 `구현/후보/선행필요`):

| 기능 | CSP | CMP | CMDP | CSC | OAM | OAM-SVC | AGENT | 계 |
|---|---|---|---|---|---|---|---|---|
| 공통 | 6/5/2 | 3/4 | 3/2/1 | 6/8 | 8/12 | 4/4 | 6/18/2 | **36/53/5** |
| CSCF | 7/8/4 | · | · | 0/1/1 | · | · | · | **7/9/5** |
| TAS | 3/1/2 | · | · | · | · | · | · | **3/1/2** |
| PTT-AS | 3/3 | 0/1 | · | · | · | · | · | **3/4** |
| IBCF | · | · | · | · | · | · | · | **·** |
| MRF | 0/4/1 | 4/9/1 | 1/10/1 | 0/1/1 | 0/2 | 1/0 | · | **6/26/4** |
| CSC | · | · | · | 0/6 | · | · | · | **0/6** |
| 관리 | · | · | · | · | 0/8/1 | 0/1 | 1/4 | **1/13/1** |
| **계** | **19/21/9** | **7/14/1** | **4/12/2** | **6/16/2** | **8/22/1** | **5/5** | **7/22/2** | **56/112/17** |

구현된 56건은 `공통`(36) 에 몰려 있다. 호처리 기능축(CSCF 7 · TAS 3 · MRF 6 · PTT-AS 3 ·
IBCF 0 · CSC 0 · 관리 1)이 비어 있다 — 지금 감시되는 것은 대부분 **프로세스·호스트·설정**
계열이고, **그 위에서 도는 통신 기능의 품질·연동 상태**는 거의 안 보인다. PTT-AS 3 은 감사
이벤트(긴급 모드·제휴·ad-hoc 그룹)이며, 알람 축은 아직 0 이다.

**정의(요구) 115건 충족**:

| 기능 | 정의 | 감지 행 | 완비 | 부분 | 무감지 | 감지 주체 미배정 |
|---|---:|---:|---:|---:|---:|---:|
| 공통 | 31 | 94 | 10 | 4 | 17 | 0 |
| CSCF | 20 | 21 | 7 | 0 | 13 | 5 |
| TAS | 5 | 6 | 1 | 1 | 3 | 2 |
| PTT-AS | 13 | 7 | 3 | 0 | 10 | 6 |
| IBCF | 8 | 0 | 0 | 0 | 8 | 8 |
| MRF | 18 | 36 | 2 | 3 | 13 | 2 |
| CSC | 6 | 6 | 0 | 0 | 6 | 0 |
| 관리 | 14 | 15 | 0 | 1 | 13 | 0 |
| **합계** | **115** | **185** | **23** | **9** | **83** | **23** |

- **완비** = 그 code 의 감지 행이 전부 `구현`.
- **부분** = `구현` 감지 행이 있으나 같은 code 에 `후보`/`후보(선행 필요)` 도 남음.
- **무감지** = `구현` 감지 행이 0건. 이 중 **26건은 감지 행 자체가 없다**(§2 감지 주체
  미배정) — 나머지 60건은 감지 방식이 CSV 에 적혀 있고 코드만 없다.

따라서 미구현 86건은 성격이 둘로 갈린다: **설계가 있고 구현만 남은 60건**과 **감지 방식부터
정해야 하는 26건**. 이행 순서를 잡을 때 둘을 섞지 않는다.

### 11.2 감지 주체 미배정 23건 — 무엇이 안 보이는가

운용 관점에서, 이 23건은 "장애가 나도 콘솔에 아무것도 안 뜨는" 구간이다. 기능축으로 묶으면
다음 네 덩어리다.

**단 2건은 장부상 결손이다** — §5 의 수렴 규칙에 따라 대표 정의 `A-COM-007` 로 발화하므로
요구 축에만 별개로 남아 있고 **운용상으로는 보인다**: `A-COM-006`(IBCF 경계 릴레이 두절)과
`A-COM-008`(PTT 미디어평면 두절). 대표의 감지 행(CSP `cmp` — 전체 CMP 두절)은 이미 `구현`이고
endpoint 단위 `cmp/<ep>` 만 후보로 남았다.

따라서 실제로 안 보이는 것은 **21건**이다.

#### (a) 대외 연동이 끊겨도 모른다 — IBCF 8건 (전부)

타 사업자·타 IMS 와 붙는 경계 구간에 감시가 하나도 없다.

| code | 안 보이는 것 | 운용자가 겪는 일 |
|---|---|---|
| `A-COM-005` | 대국 OPTIONS keepalive 무응답 | 상대 사업자 쪽이 죽어도 알람이 없다. 그 경로 호가 전부 실패한 뒤 가입자 민원으로 안다 |
| `A-QOS-012` | 대국별 호 성공률 하한 | 특정 대국만 품질이 무너져도 전체 평균에 묻힌다 |
| `A-QOS-013` | 대국별 특정 실패코드(503/486/408) 급증 | 대국 과부하·번호 오류가 진행 중인데 사후 통계로만 보인다 |
| `A-QOS-014` | 대국별 신호 RTT 임계 | 전송망 지연이 타임아웃으로 번지기 전에 잡을 수단이 없다 |
| `A-COM-006` | 경계 미디어 릴레이 제어 두절 | (CSP 동거 배치는 `A-COM-007` 로 수렴 — §5) |
| `A-SEC-002` | 사기 호·스캐너 급증 | 요금 부정사용(toll fraud)이 진행돼도 탐지가 없다 |
| `E-AUD-007` | 대국 수동 차단/해제 | 누가 언제 대국을 막았는지 감사 기록이 없다 |
| `E-STC-006` | 대국 가용 상태 전이(alive↔dead) | 경로가 오르내린 이력이 안 남는다 |

**공통 선행**: 대국 상태를 판정하는 장치 자체가 없다 — `MarkAlive`/`MarkFail` 호출자 0건,
`RouteRuntime.alive` 는 항상 true(§10.1 `peer/<remote_node>`). OPTIONS keepalive 프로버를
만들면 `A-COM-005`·`E-STC-006` 이 한 번에 열리고, 그 위에 대국별 집계(012~014)가 얹힌다.
이것은 알람 결손인 동시에 **failover 기능 결함**이다 — 죽은 대국으로 계속 호를 보낸다.

#### (b) PTT 알람 축이 비어 있다 — PTT-AS 6건

감사 이벤트 3종(`E-STC-007` 긴급 모드 · `E-AUD-009` 제휴 변경 · `E-AUD-010` ad-hoc 그룹)은
구현됐다(§10.1). **남은 것은 전부 알람 축**이다.

| code | 안 보이는 것 | 운용자가 겪는 일 |
|---|---|---|
| `A-QOS-017` | **긴급·임박위험 호 처리 실패** | 인명 직결 기능이 실패해도 무알람. 소수 건도 즉시 떠야 하는 등급이다 |
| `A-COM-009` | GMS/CMS/IdMS 연동 두절 | 프로비저닝·affiliation·인증 갱신이 멈춘 걸 모른다 |
| `A-QOS-016` | 그룹콜 수립 성공률 하한 | 그룹 통신이 안 되기 시작해도 임계 경보가 없다 |
| `A-QOS-018` | affiliation/그룹 수용 한계 도달 | 신규 그룹 가입이 거절되는데 원인이 상한인지 장애인지 구분이 안 된다 |
| `A-PRC-015` | KMS 키 자재 만료 임박·획득 실패 | 암호화 그룹 통신이 어느 날 갑자기 불능이 된다 |
| `A-COM-008` | 미디어평면 두절 → floor 제어 불가 | (CSP 동거 배치는 `A-COM-007` 로 수렴 — §5. 운용상으로는 보인다) |

**가장 먼저 볼 것은 `A-QOS-017`** — severity 가 critical 이고 성격이 safety_critical_failure
다. 다른 것들과 달리 "품질이 나빠졌다"가 아니라 "사람이 위험할 수 있다"는 조건이다.
`E-STC-007`(긴급 모드 전이)이 이미 이벤트로 남으므로, **그 전이 중 실패한 것**을 세어
임계를 거는 것이 자연스러운 다음 걸음이다.

#### (c) 등록·라우팅의 원천이 끊겨도 모른다 — CSCF 5건

| code | 안 보이는 것 | 운용자가 겪는 일 |
|---|---|---|
| `A-COM-002` | 가입자 원천(CSC) 연결 두절 | 신규 등록·인증·프로파일 조회가 전면 실패하는데 알람이 없다 |
| `A-COM-004` | DNS/ENUM 해석 실패 지속 | 도메인 기반 라우팅·번호 변환이 막힌 원인을 못 짚는다 |
| `A-QOS-008` | 동시 세션 사용률 단계 임계 | 상한에 닿아 신규 호가 전면 거절되기 **전에** 경고가 없다 |
| `A-QOS-010` | 다이얼로그/트랜잭션 자원 고갈 | 세션 누수가 쌓여 수용 불가가 될 때까지 안 보인다 |
| `A-SEC-001` | 인증 실패 급증 | 무차별 대입·크리덴셜 스터핑이 진행돼도 탐지가 없다 |

`A-QOS-008`·`A-QOS-010` 은 **수용 상한값이 정의돼 있어야** 비율을 낼 수 있다 — 상한 신설이
선행이다(§11 절차 2단계). `A-SEC-001` 은 카운터/윈도 집계가 선행.

#### (d) 품질 지표가 없다 — MRF 2건 · TAS 2건

| code | 안 보이는 것 |
|---|---|
| `A-QOS-021` | 활성 세션 중 **미디어 무흐름** 비율 — NAT/방화벽 문제로 무음 호가 퍼져도 시그널링은 정상으로 보인다 |
| `A-QOS-022` | 패킷 손실·지터 임계 — RTCP 수집이 선행 |
| `A-QOS-015` | 부가서비스(전환·착신전환·발신제한) 실행 실패율 |
| `E-AUD-008` | 부가서비스 설정 변경 감사 |

#### 정리 — 성격별 선행 조건

| 성격 | 해당 | 선행 |
|---|---|---|
| 상태 프로버 신설 | IBCF `A-COM-005`·`E-STC-006` | OPTIONS keepalive 프로버 (failover 결함과 동일 뿌리) |
| 상한값 정의 | `A-QOS-008`·`A-QOS-010`·`A-QOS-018` | 수용 상한 설정 키 신설 — 없으면 비율 산출 불가 |
| 카운터·율 집계 | `A-QOS-012`~`014`·`A-QOS-016`·`A-SEC-001`·`A-SEC-002`·`A-QOS-015` | 윈도 집계 카운터 (§6 — 일회성 오류는 알람 부적합, 율로 전이를 만들어야 한다) |
| 품질 수집 | `A-QOS-021`·`A-QOS-022` | 미디어 무흐름 판정·RTCP 손실/지터 수집 |
| 외부 의존 probe | `A-COM-002`·`A-COM-004` | 원천/리졸버 도달성 주기 검사 |
| 자재 수명 | `A-PRC-015` | KMS 연동·만료 조회 |
| 감사 이벤트 배선 | `E-AUD-007`·`E-AUD-008` | 발생 지점에서 event_log 기록 — 임계·집계 없이 배선만으로 닫힌다 |

감사 이벤트 배선이 가장 싸다 — 새 수집기도 임계도 필요 없고, 이미 그 동작을 하는 코드 자리에
기록 한 줄을 넣는 일이다. `E-STC-007`·`E-AUD-009`·`E-AUD-010`(CSP)은 그렇게 닫았다(§10.1).

잔여 두 건은 배선 전에 정할 것이 있다:

- **`E-AUD-007` 대국 수동 차단** — 차단 수단은 이미 있다(`remote_nodes.enabled`, CSP 가
