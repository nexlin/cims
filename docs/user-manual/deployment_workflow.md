...
- **oam-svc 를 올린 뒤에는 OAM 을 한 번 재기동한다** — 콘솔 번들 경로는 OAM 기동 시
  **1회만 해석**되므로(`console_static.resolve`, oam-svc 동봉 콘솔 최신 버전 우선), 재기동
  전까지 콘솔은 이전 oam-svc 버전의 UI 를 계속 서빙한다. API 라우트는 즉시 반영되고 화면만
  이전 것이라 알아채기 어렵다 — 서빙 중 번들 해시(`GET /` 의 `index-*.js`)를 설치본과
  대조하면 확실하다.
