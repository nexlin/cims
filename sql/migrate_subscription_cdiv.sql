...
--      forward_no_reply_id       CFNR — 착신 leg 가 링잉 뒤 forward_no_reply_sec(0 = Setup.Sip.Cdiv.NoReplySec) 안에 응답하지 않을 때(링잉 뒤 480/408 포함)
--      forward_not_logged_in_id  CFNL — 착신 가입자가 미등록일 때(INVITE 시점)
--      forward_not_reachable_id  CFNRc — 등록은 돼 있으나 도달 불가(Q.850 cause 20, 링잉 없이 480/408 — 무선 이탈·NAT 바인딩 소실)
...
BEGIN
    ...
    IF has_tbl = 1 THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = tbl AND COLUMN_NAME = 'forward_not_logged_in_id') THEN
            ...
            PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = tbl AND COLUMN_NAME = 'forward_not_reachable_id') THEN
            SET @sql := CONCAT('ALTER TABLE ', tbl, ' ADD COLUMN forward_not_reachable_id VARCHAR(64) NOT NULL DEFAULT '''' COMMENT ''CFNRc 전환 대상(도달 불가, TS 24.604)''');
            PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
        END IF;
    END IF;
END //
