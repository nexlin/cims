...
--    passwd 컬럼을 읽지 않는 코드(CSP SELECT·cspsim -db·CSC admin·verify 시드)가 배포된 상태에서 적용한다.
--    현재 코드는 passwd 를 읽지도 쓰지도 않으므로 DROP 은 코드 변경 없이 이 스크립트만 적용하면 된다
--    (역순 — 구 코드 위에서 DROP 하면 CSP 기동 질의가 깨진다).
