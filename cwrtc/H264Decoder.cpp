...
CH264Decoder::CH264Decoder()
    : m_pSvcDecoder(NULL), m_bInit(false), m_pRGB(NULL), m_iWidth(0),
      m_iHeight(0) {}

CH264Decoder::~CH264Decoder() { Destory(); }
...
bool CH264Decoder::Create(const char *pszDllName) {
  if (CreateDecoder == NULL || DestroyDecoder == NULL) {
    ...
    CreateDecoder =
        (H264CreateDecoder)GetProcAddress(hDll, "WelsCreateDecoder");
    if (CreateDecoder == NULL)
      return false;

    DestroyDecoder =
        (H264DestroyDecoder)GetProcAddress(hDll, "WelsDestroyDecoder");
    if (DestroyDecoder == NULL)
      return false;
  }
...
bool CH264Decoder::Decode(uint8_t *pInput, int iInputLen) {
  ...
  if (m_pSvcDecoder->DecodeFrame2(pInput, iInputLen, arrBuf, &sttBufInfo) !=
      0) {
    return false;
  }
