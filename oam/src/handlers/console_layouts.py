...
def installed_services(config: dict) -> set:
    """가용 서비스 모듈 집합.
      분리 배포(--role base): 게이트웨이 라우트 테이블의 module 들.
      단일 프로세스(all, 라우트 없음): in-process 라 알려진 서비스 전부 가용.
    """
    mods = set()
    if _gateway is not None:
        try:
            mods = {r.get('module') for r in _gateway.enabled_routes(config) if r.get('module')}
        except Exception:
            mods = set()
    if not mods:
        mods = set(_KNOWN_SERVICES)
    return mods
