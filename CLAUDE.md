...
- [dispatch_center.md](docs/design/features/dispatch_center.md) — 관제 센터 (관제 그룹 엔티티 = 픽업 그룹+대표번호+감청 범위 — `pickup_group` 값을 그룹 id 로 파생해 한 축 유지, 가입자당 그룹 1개, 대표번호 병렬 호출 = TS 24.239 Flexible Alerting(포크 집합·최초 200 승자·RELAY_MODIFY 고정), **업무망 합법감청**(TS 33.107/33.108·ETSI 정합 — CC 분리 인도·귀속 보존, 서버 믹싱 미규정) = RFC 4235 dialog 이벤트 범위 확장 + RFC 3911 Join `a=recvonly` → CMP 청취 leg `RELAY_TAP_*`(양 peer 를 SSRC 2개 분리 인도·RFC 5576 라벨링·믹싱은 단말·상향 폐기·은닉), PTT 그룹콜 청취 = `recv_only`/`floor_suppress` JOIN + TS 24.484 `ptt_user_profile.allow_ambient_listening` 자격 + `ptt_listen` 범위 2단 인가(규격형), 로스터 노출 `listen_visibility` 은닉/투명 양지원, 감사 `E-AUD-016`. 검증 `S3-SCN-FA`/`S3-SCN-MONITOR`. **③ 관제 그룹 모델·② 대표번호 병렬 호출(TAS `CTasForkSet`·cspsim `hunt`)·① 업무망 합법감청(TAS `HandleIncomingJoin`→CMP `PRtpTap` 청취 leg·dialog 인가 범위·SSRC 2개 분리·은닉·감사 `E-AUD-016`·cspsim `monitor`) 구현·실측 완료 — 남은 것: PTT 그룹콜 청취 JOIN·감사 화면·단말 파트**)
