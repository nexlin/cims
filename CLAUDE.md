...
- [dispatch_desktop_ui.md](docs/design/features/dispatch_desktop_ui.md) — 관제조작반 앱 UI(Windows WPF `windows/dispatch-desktop`, Android 태블릿 밀도 §11) — 화면 한 장(1920×1080 무스크롤) 위 다섯 구획(① 통화 보드 BLF · ② 대표번호 데스크(대기열·내 통화·다이얼) · ③ 감청 패널(caller/callee 분리 미터·은닉 표시) · ④ PTT 채널(멤버/청취 카드·PTT 버튼·긴급 배지) · ⑤ SDS·디렉터리), 착신/긴급 배너·토스트, 로그인·프로비저닝 흐름, 오디오 라우트 UI(헤드셋/스피커), 핫키, 응답 코드→문구 사전, MVVM 구조. 설계 정본 — 구현 전
