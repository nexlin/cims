...
| **CMP** (`cmp/`) | RTP relay + MCPTT floor control (CSP 의 UDP JSON 제어) + 피어 leg 한정 G.711↔AMR-WB 트랜스코딩(cmp.md §11 — CSP 코덱 삽입·`media_codec`, CMP `PTranscoder`·`TranscodeSlots`, IP-PBX 트렁크용) + **안내 재생기**(cmp.md §12 — `RELAY_PLAY` leg 재생 원천, 동봉 음원 `announcements/sys`, MRFP 역할) | [docs/design/modules/cmp.md](docs/design/modules/cmp.md) |
...
- [volte_supplementary_services.md](docs/design/features/volte_supplementary_services.md) — 유선 VoIP 보조 서비스 (데스크폰·소프트폰·관제 앱 — USIM 없는 Digest+TLS 가입자 규약(가입 id=E.164, 내선=표시 라벨), 유선 접속서비스 `kind=voip` 에 피처코드·전달 기본값, 당겨받기 축 `pickup_group`=전화 그룹 id(org 폴백 없음)·서비스별 `pickup_feature_code`(전역 `CallPickupId` 제거됨)·지정/그룹 픽업, 호 전달 REFER(blind/attended) — 미디어 재고정은 전부 CMP `RELAY_MODIFY`·SRTP 유지, 표준형=수신 INVITE-Replaces(RFC 3891)·dialog 이벤트 패키지(RFC 4235 BLF)·489, **착신전환 §6A** = 서버측 CDIV(TS 24.604 — `forward_id` CFU, 181·History-Info·전환 안내, 302 없음). 검증=S3-SCN-XFER/PICKUP/DIALOG. 보조 서비스 로직은 CTasModule 소유(IModule 훅), relay SDES leg 헬퍼는 MediaSdes 공용. `voip` 귀속·폴백 제거 구현 반영 — 기존 회선 `voip` 이관(H(A1) 재결박) §10.3a)
...
- [sip_statistics.md](docs/design/features/sip_statistics.md) — SIP 호·메시지 통계 (세는 단위 3계층 attempt/session/leg — 그룹통화·감청·PBX 로 leg 이 늘어도 비율이 흔들리지 않게. 성공률·소통률·완료율·참여율, 1분 기저 롤업 피라미드(분~월 정수배 유도), 미결 호 추적, 보존 계층. 구현: 서비스 판정 단일화(`services/access_services`)·VoLTE 3지표·1분 기저 집계(`services/stats_rollup`, oam-svc 주기 실행). 조회 API(§7 — 통일 버킷 키·granularity·coverage·재집계 202 job)·PTT 시도 장부(Y6) 구현 반영)
- [sip_service_model.md](docs/design/features/sip_service_model.md) / [sip_runtime_config.md](docs/design/features/sip_runtime_config.md) — SIP 서비스 모델·런타임 설정 (**다이얼 플랜 §2-10** = 착신 번호 번역 TS 24.229 §5.4.3.2 — 접속서비스·피어 Route 의 `country_code/national_prefix/international_prefix`, Request-URI 만 재작성·484·`CspDialPlan`·S1-UNIT-CSP; 접속서비스 `kind` = 접속환경 클래스 `volte` 이동 / `voip` 유선 / `ptt` — `voip` 구현 반영: CSP kind 검증·같은 전화 경로·로그/통계 서비스축 합산 `LogServiceOf`, CSC `service_ref`↔서비스 name 매칭 프로비저닝, 관제 앱 전화 회선 voip 우선. **CSC 접속서비스 읽기 경로 단일화** `services/access_services` — 정의는 관리 store 미러(CSP 정본) · csc.json `Provisioning.Services` 는 단말 도달 정보+미러 부재 폴백. 가입 테이블 = kind(`volte_subscriptions`/`voip_subscriptions`/`ptt_subscriptions`, exact-kind 게이트))
...
- [site_directory_layout.md](docs/design/features/site_directory_layout.md) — **사이트 디렉터리 레이아웃** — `CimsSiteDir` 하나에서 runtime/packages/content/recordings/log/stats/state/tester 여덟 영역을 유도(영역 키 명시값 우선, 유도 정본 `services/paths.py`). 모듈은 템플릿 `site_area` 로 쓰는 영역을 선언하고 실체화가 base oam 값으로 채운다(모듈별 경로 입력 없음). 영역 안 고정 이름 = log/sip·recordings/{volte,ptt,message}·stats/{ptt_index,ptt_attempts}·state/{volte,ptt}·content/{announcements,mcdata_fd}. 비우면 단일 루트 레이아웃(옛 사이트 경로 그대로). 부트스트랩 `install.sh --site-dir`, HA 이관은 사이트 전체를 옮긴다. 위치는 경로 설정(NAS·로컬 무관), 마운트는 인프라의 일·`CimsRuntimeMount` 는 선택 guard
...
- [announcements.md](docs/design/features/announcements.md) — **안내음성·신호음·보류 음악**(P1 구현·실측 반영 — 없는 번호 404·무응답 408 early media 안내 pass) — CSP=MRFC(상황 판정 Reason Q.850 우선·프로파일 해석 가입자→접속서비스→전역·상태 머신) / CMP=MRFP(relay leg 재생기 `RELAY_PLAY`·`RELAY_PLAY_DONE`·`resource.ann`, 워커 timerfd 20 ms 페이싱, 코덱별 사전 인코딩 파일). 실패 안내 = early media 모델(183+SDP `a=sendrecv`+`P-Early-Media` → 재생 → **원래 최종 코드 그대로**, TS 24.628·RFC 3960), 보류 MOH(TS 24.610 — 재협상 없이 원천 교체), 서버 링백 기본 off(접속서비스 스위치·가입자 링백 자리). 서비스 음원 라이브러리(`sys:`/`op:`/`sub:` — 계측기 샘플과 형식·변환기만 공유, 저장소·API·콘솔 `/service/announcements` 별개, agent `/module-file` 배포·SIGUSR1 재적재, 업그레이드 때 `announcements/op` 이어받기). 내장 프로파일 `default`·`trunk`·`ringback`(서버 링백 스위치), 계측기 `VOLTE-ANN-*` 6종(`moh_rtp_pct`·subscriber 픽스처 `ringback_media`·`forward_to`). **착신전환 안내(TS 24.604 CDIV)** = 302 리다이렉트 폐기 → 서버측 전환(TAS `ResolveDiversion` CFU 연쇄·상한 `Setup.Sip.Cdiv`·루프 486, 발신자 181, B-leg `History-Info` RFC 7044 cause=302, CDR `diversion`) + 상황 `forwarded`(기본 `announce_then_tone` — `sys:ann_forwarded` 뒤 링백음을 응답까지, `OnForwarded`) — volte_supplementary_services.md §6A·announcements.md §3.5, `VOLTE-ANN-FORWARDED`(`cdiv_181_pct`·`cdiv_hi_pct`). **조건부 전환 CFB/CFNR/CFNL/CFNRc**(§6A.4 — 가입 컬럼 5종·CFNR/CFNRc 는 링잉 여부(`m_bRang`)로 구분·`TryDivertLeg` 같은 relay 위 재타게팅·CFNR 시한 `Setup.Sip.Cdiv.NoReplySec`)·**`sub:` 가입자 링백 업로드**(OAM scope op/sub·콘솔 [가입자 링백…])·**통화중대기 in-band 대기음**(CMP `mode=mix` `PAnnMixer`·상황 `call_waiting_alert`·프로파일 `cw_inband`, §3.6) 구현. P2 잔여 = 큐/ACD·피어 대상 조건부 전환·실단말 청취
