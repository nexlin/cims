...
- [volte_supplementary_services.md](docs/design/features/volte_supplementary_services.md) — 관제 소프트폰 보조 서비스 (USIM 없는 Digest+TLS 가입자 규약(내선=가입 id=imsi), 당겨받기 `pickup_group` 축·서비스별 피처코드·지정/그룹 픽업, 호 전달 REFER(blind/attended) — 미디어 재고정은 전부 CMP `RELAY_MODIFY`·SRTP 유지, 표준형=수신 INVITE-Replaces(RFC 3891)·dialog 이벤트 패키지(RFC 4235 BLF)·489. 설정=접속서비스 `pickup_feature_code`/`transfer_allowed` + 가입자 `pickup_group`. 검증=S3-SCN-XFER/PICKUP/DIALOG(`pickup_group` 실컬럼 축 + 403/404/489 게이트 자동 판정 + D5 미등록 SUBSCRIBE Digest 수락). P0~P3 구현 반영 — 보조 서비스 로직은 CTasModule 소유(IModule 훅), relay SDES leg 헬퍼는 MediaSdes 공용)
- [mcptt_authorization.md](docs/design/features/mcptt_authorization.md) — MCPTT 권한/RBAC
- [dispatch_center.md](docs/design/features/dispatch_center.md) — 관제 센터 (관제 그룹 엔티티 = 픽업 그룹+대표번호+감청 범위 — `pickup_group` 값을 그룹 id 로 파생해 한 축 유지, 가입자당 그룹 1개, 대표번호 병렬 호출 = TS 24.239 Flexible Alerting(포크 집합·최초 200 승자·RELAY_MODIFY 고정), **업무망 합법감청**(TS 33.107/33.108·ETSI 정합 — CC 분리 인도·귀속 보존, 서버 믹싱 미규정) = RFC 4235 dialog 이벤트 범위 확장 + RFC 3911 Join `a=recvonly` → CMP 청취 leg `RELAY_TAP_*`(양 peer 를 SSRC 2개 분리 인도·RFC 5576 라벨링·믹싱은 단말·상향 폐기·은닉), PTT 그룹콜 청취 = 관제사의 `a=recvonly` INVITE 합류 → `PTT_JOIN recv_only=1` + TS 24.484 `ptt_user_profile.allow_ambient_listening` 자격 + `ptt_listen` 범위 2단 인가(규격형), 로스터 노출 `listen_visibility` 은닉/투명 양지원, 비멤버 sendrecv 403, 감사 `E-AUD-016`(콘솔 `장애>감사 이력` manager 열람·OAM `/events` audit 게이트). sequential alerting(단계 시한 큐)·링잉 대표번호 호 당겨받기(`PickUpFork`). 관제 그룹 **관리 범위** `directory_admin`(none|own|all, §3.4) = 관제사가 관제 앱에서 조직/구성원/번호·PTT 그룹을 관리하는 인가 축(콘솔 manager 부여, 콘솔 관리 API 와 같은 쓰기 코드) + 녹취 열람 §5.7b. 검증 `S3-SCN-FA`(F1·F3·F5·F6·F7 dialog 정합)/`S3-SCN-MONITOR`/`S3-SCN-PTT-LISTEN`(L1~L5). **서버·콘솔·검증 전부 구현·실측 완료 — 남은 것: 단말 파트(Join 발신·SSRC 디먹스 U10·PTT 청취 채널 UI U6)와 §10 향후 과제**)
...
- [dispatch_desktop_ui.md](docs/design/features/dispatch_desktop_ui.md) — 관제조작반 앱 UI(Windows WPF `windows/dispatch-desktop`, Android 태블릿 밀도 §12) — **PTT 채널 중심**, 화면 한 장(1920×1080 무스크롤)을 좌 PTT · 우 일반통화 같은 폭 두 열로: 각 열 = 위 운영 · 아래 실시간 내역, 운영 안 = 왼쪽 운영 | 오른쪽 발신+주소록 / 아래 메시지. 네 도킹 패널(① PTT 채널: 멤버·사설콜·애드혹 카드 + PTT 발신(사설콜/애드혹, PTT 주소록) + MCData 메시지 · ② PTT 내역 · ③ 일반통화: 관제 그룹원 상태 띠(2~4명)·대표번호 대기열·내 통화 + 다이얼패드·주소록 발신 + SMS·LMS(SIP MESSAGE text/plain, 외부망 게이트웨이는 서버 과제) · ④ 일반통화 내역) + **감청 창**(듣기만 하는 세션은 팝업 — 내역 행·그룹원 띠의 [청취]), 도킹(이동·별창·접기·잠금·프리셋), 착신/긴급 배너·토스트, 로그인·프로비저닝, 오디오 라우트, 핫키, 응답 코드→문구 사전, MVVM 구조. 서버 전제 = 청취 범위 그룹 conference 구독 인가(dispatch_center §10). **WPF 앱 구현·빌드 완료**(§11 구조 그대로 — 세션 호스트 `DispatchSession` 이 코어 투영, ViewModel 12·패널 뷰 4·감청/로그인/설정 창, AvalonDock 도킹·프리셋, `--ui-preview` 개발 스위치) — 개발 서버 실기 e2e 통과(로그인·등록·대표번호 착신/응답/부재/동료 응답·A/B-leg 종료·PTT 청취·SDS·그룹 CRUD·핫키, 남은 것 = 오디오 실청취). **PTT 그룹 생성·편집·삭제** = 관제사(가입자)가 GMS XCAP PUT/DELETE(TS 24.481, 자격 `ptt.allowCreateGroup`·본인 소유) — 앱 `GroupEditWindow`·`RefreshGroupsAsync`·xcap-diff 자동 재조회, SDK `CscClient.putGroup/deleteGroup`+`GroupDoc`. 활성 세션 발견은 프로비저닝 `dispatch.members[]/pttTargets[]`(계약 [docs/design/features/android_ue_provisioning.md](docs/design/features/android_ue_provisioning.md) §3). **상단 바 최상위 메뉴 4개** §3.4 = [관제 F1](도킹 캔버스) · [이력 F2](§4.6 끝난 세션 날짜 창 조회 `/provisioning/history?until=` + 녹취 재생 `/provisioning/recordings/*` = CSC 범위 게이트 + oam-svc 프록시, MP4/AAC) · [PTT 그룹 F3](§4.7 범위 안 전부, GMS XCAP) · [관리 F4](§4.5 조직/구성원/VoLTE·PTT 번호, 관제 그룹 `directory_admin` 범위 own|all, `/provisioning/directory/*` — 범위 없으면 비활성+툴팁) — 별창이 아니라 같은 창의 레이어 전환(도킹 호스트 상시 마운트), 관제 밖 화면 위 **관제 요약 띠** §3.5(선택 채널·발언 상태·PTT·대기열·내 통화·감청 창·[관제로]), 세션을 만드는 조작은 관제로 자동 복귀, 화면 [↗ 별창으로](`ScreenWindow`), 화면 VM 은 앱 수명 동안 하나(폼 유지)
