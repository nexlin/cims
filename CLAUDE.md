...
| **계측기** (`ems/tester/`, `tester/`) | `oam-cims-tester` — OAM 게이트웨이 뒤 서비스 모듈(`oam-svc` 와 동격, loopback 4490) + `cims-tester-worker`(C++ 부하·피어 워커) + 콘솔 팩(`시험` 그룹). N UE 부하 + IBCF/IP-PBX/MGCF 피어 시뮬레이터. 독립(자기 `oam`)/동거 두 운영 형태, 워커는 시험 대상 밖 호스트. **A·B 단계 구현** = 계약 모델(pydantic→`schema/*.json`)·컨트롤러 API·콘솔 팩·게이트웨이 SSE 통과·nav 서비스 게이팅 + `libcsim`(cspsim 승격)·워커 ue 풀/단계 실행기/1초 집계 스트림·컨트롤러 run 오케스트레이션(프로파일·요약·판정·보고서)·CLI `run/report/creds-from-db`. **C 단계** = `libcsim` 피어 엔진 `CsimPeer`(고정 수신점·신원 범위·다수 동시 호, ibcf/pbx/mgcf 코덱 프로파일)·워커 `peer` 풀·대상 CSP 컬렉션 시드/복원(토폴로지 피어 풀에서 파생, OAM 컬렉션 API, 피어링 접속점 시드·ACL scope=local_node)·IBCF in/out·ACL·failover 시나리오. **D 단계** = pbx/mgcf 프로파일 — 183 early media·100rel/PRACK, hold/resume(re-INVITE), blind REFER, RFC 4733 DTMF(libcsim RTP 송수신), Reason Q.850 송신·관측, pbx 트렁크 REGISTER, 번호 prefix 시드 규칙(pbx/mgcf 는 번호 그대로 다이얼), 단계 `progress/hold/resume/dtmf/refer`·`cause`, 비율 지표 4종. 실측 7 pass, fail 3 은 CSP 과제(§12 Reason 미투과·503→603·트렁크 계정). **E 단계** = 콘솔 팩 5 라우트(토폴로지 편집·연결 검사·워커 상태 / 시나리오·프로파일 YAML 편집기 / 실행 라이브 SSE 차트·중단·율 조정 / 결과 보고서·인쇄 / 비교 회귀 판정) + 컨트롤러 편집·검사·시계열·비교·api-docs API. **토폴로지 모델 확정** = 호스트›워커·대상 노드›풀 3단(주소는 호스트에만, 대상 = 역할별 노드 집합 sip/tas/media/subscriber/oam/db — CIMS·타 IMS·IP-PBX 공통, 동거 허용) + DnD 캔버스 UX 목업 라우트 `/test/topology-canvas`. **시나리오 UX 확정(목업 `/test/scenario-canvas`)** = 레인(역할)×행(단계) 시퀀스 캔버스 + 팔레트 + 속성 패널 + 드로어(YAML 양방향·검증·토폴로지 적합성 드라이런·절차표), 세션 열(당사자별 송출 가닥), 모델 확장 제안 = `media_hold.during`·`invite.media.rtp auto|none|explicit`·`media_send/stop`·샘플 라이브러리(test_instrument.md §7). **실행·결과·비교 UX 확정(목업 `/test/run-live`·`/test/run-report`·`/test/run-compare`)** = 워커·대상 띠 + 단계 사다리·종료 조건 게이지·지표별 소형 차트(기대치 점선)·절차 진행·SIP 사다리 드로어·필터/날짜 그룹 색인·재실행·시작 창 계획 미리보기(Little 검산) / run 레일 + 판정 요약(왜 FAIL)·알람 겹침·히스토그램·여유 막대·대상 증거 / run 카드·Δ 막대 매트릭스·t+0 정렬 시간축 겹침·기대치 diff·추세 모드, 제안 API = `runs/plan`·`hold`·`hist`·`sip/{call_id}`·`target-alerts`·색인 필터·`compare?format=`. 남은 것 = 토폴로지 v2 컨트롤러 이행·캔버스/시나리오 정식 편집기(vocab·compile-check API)·미디어 평면 워커 이행·히스토그램 버킷·알람 타임라인·S3/S6 항목 이전·워커 2대 분산 | [docs/design/features/test_instrument.md](docs/design/features/test_instrument.md) |
