...
| **CMP** (`cmp/`) | RTP relay + MCPTT floor control (CSP 의 UDP JSON 제어) + 피어 leg 한정 G.711↔AMR-WB 트랜스코딩(cmp.md §11 — CSP 코덱 삽입·`media_codec`, CMP `PTranscoder`·`TranscodeSlots`, IP-PBX 트렁크용) | [docs/design/modules/cmp.md](docs/design/modules/cmp.md) |
...
- [announcements.md](docs/design/features/announcements.md) — **안내음성·신호음·보류 음악**(설계 정본, P1 착수 전) — CSP=MRFC(상황 판정 Reason Q.850 우선·프로파일 해석 가입자→접속서비스→전역·상태 머신) / CMP=MRFP(relay leg 재생기 `RELAY_PLAY`·`RELAY_PLAY_DONE`·`resource.ann`, 워커 timerfd 20 ms 페이싱, 코덱별 사전 인코딩 파일). 실패 안내 = early media 모델(183+SDP `a=sendrecv`+`P-Early-Media` → 재생 → **원래 최종 코드 그대로**, TS 24.628·RFC 3960), 보류 MOH(TS 24.610 — 재협상 없이 원천 교체), 서버 링백 기본 off(접속서비스 스위치·가입자 링백 자리). 서비스 음원 라이브러리(`sys:`/`op:`/`sub:` — 계측기 샘플과 형식·변환기만 공유, 저장소·API·콘솔 `/service/announcements` 별개, agent `/module-file` 배포·SIGUSR1 재적재). P2 = 통화중대기(TS 24.615·RFC 7462)·착신전환 안내·mix 모드·큐/ACD
