...
- [dispatch_desktop_ui.md](docs/design/features/dispatch_desktop_ui.md) — 관제조작반 앱 UI(Windows WPF `windows/dispatch-desktop`, Android 태블릿 밀도 §12) — 화면 한 장(1920×1080 무스크롤) 위 다섯 구획(① 통화 보드 BLF · ② 통화 데스크(대표번호 대기열·내 통화·**발신 3모드** VoLTE/PTT 사설콜/PTT 애드혹) · ③ 활성 세션(VoLTE 통화 목록 + PTT 세션 목록 — 행 안 [청취]) · ④ PTT 채널(발언하는 세션 — 멤버·사설콜·애드혹 카드) · ⑤ SDS·디렉터리) + **감청 창**(듣기만 하는 세션은 전부 팝업 — VoLTE caller/callee 분리 미터·PTT 청취), 착신/긴급 배너·토스트, 로그인·프로비저닝 흐름, 오디오 라우트 UI, 핫키, 응답 코드→문구 사전, MVVM 구조. 서버 전제 = 청취 범위 그룹 conference 구독 인가(dispatch_center §10). 설계 정본 — 구현 전
