...
- [sip_access_security.md](docs/design/features/sip_access_security.md) — SIP 접속 보안 (TS 33.203 정합 로드맵 P0~P4. P0=채널 정책 게이트(`sip_transport=TLS` 집행, psip listener id 전 transport 전파), P1=인증 자료 경계(H(A1) SoT·CSC 단일 쓰기 주체·Digest 정비·`/provisioning/me` sipHa1). P0·P1 구현 반영 — `passwd` 소거·DROP 은 §4.7 배포 순서)
