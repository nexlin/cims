...
- [site_directory_layout.md](docs/design/features/site_directory_layout.md) — **사이트 디렉터리 레이아웃(설계 정본, 구현 예정)** — `CimsSiteDir` 하나에서 runtime/packages/content/recordings/log/stats/state/tester 여덟 영역을 유도(영역 키 명시값 우선), 모듈의 고정 하위 이름 제거. 위치는 경로 설정(NAS·로컬 무관), 마운트는 인프라의 일·`CimsRuntimeMount` 는 선택 guard
