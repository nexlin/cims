      ...
      {editing ? (
        <FieldRow>
          ...
          <Field label={`관리 범위${scopeLocked ? ' (manager)' : ''}`} w={130}>
            <select className="form-input" disabled={scopeLocked} title="관제 앱에서 조직·구성원·VoLTE/PTT 번호·PTT 그룹을 관리할 수 있는 범위 (own = 소속 조직과 그 하위). manager 만 변경" value={form.directory_admin || 'none'} onChange={e => setForm({ ...form, directory_admin: e.target.value as DirectoryAdmin })}>
              {(Object.keys(ADMIN_LABEL) as DirectoryAdmin[]).map(k => <option key={k} value={k}>{ADMIN_LABEL[k]}</option>)}
            </select>
          </Field>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', fontSize: 12 }}>
          ...
          <span className="ts">관리 범위 {ADMIN_LABEL[existing.directory_admin || 'none']}</span>
