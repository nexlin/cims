...
const ScenarioCanvas = forwardRef<ScenarioCanvasHandle, {
  ...
  const stepLabel = (s: Step) => {
    switch (s.step) {
      ...
      case 'fd_send': return `FD ${s.payload ?? ''} ${s.to ? `→ ${s.to}` : `그룹${(s as Step).group ? ` ${(s as Step).group}` : ''}`}`; case 'fd_recv': return s.payload === 'signal' ? 'FD 도착 대기' : 'FD 도착 → 다운로드'
...
function ProcedureTable({ doc, vocab, plan }: { doc: Doc; vocab: ScenarioVocab | null; plan: PlanResult | null }) {
  const proc = (s: Step | During): string => {
    ...
    switch (s.step) {
      ...
      case 'fd_send': return `${a} 가 ${s.to ?? '그룹'} 에 MCData FD 파일 배포 — IdMS 토큰 → POST /mcdata/fd(${s.payload ?? '?'}) → FD SIGNALLING MESSAGE(FILEURL)`; case 'fd_recv': return `${a} 가 FD SIGNALLING 을 받을 때까지${s.payload === 'signal' ? '' : ' → FILEURL 다운로드·크기 대조'}`
...
function Inspector({ doc, sel, setSel, mutate, topo, vocab, issues, allIssues, isTopoIssue, docErrN, topoErrN, canWrite, source, bind, plan, onRemoveRole, onRemoveStep, onDupStep, onDuringToStep, onStepToDuring }: {
  ...
  return <div>
    ...
    {s.step === 'fd_send' && <><F label="파일 원천 (payload)" help="합성 크기(`65536`·`256k`·`2m`, 1 B ~ 64 MiB — 내용은 결정적 패턴) 또는 워커 Media.SampleDir 의 파일 이름. CSC McDataFd.MaxBytes(기본 50 MB)를 넘으면 413. to 역할 = 1:1(one-to-one-fd) · to 없음 = 그룹 FD(인스턴스 그룹 또는 group — 수신자는 multi 역할, CSC 가 allow_fd·멤버십 게이트)"><Txt value={s.payload} mono placeholder="256k" disabled={ro} onCommit={v => ST(x => { x.payload = v })} /></F>
      <F label="group" help="그룹 FD 의 그룹 id 직접 지정 — 생략 = 인스턴스가 잡은 affiliation 그룹(to 가 있으면 무시)"><Txt value={(s as Step).group} mono placeholder="(인스턴스 그룹)" disabled={ro} onCommit={v => ST(x => { if (v.trim()) (x as Step).group = v.trim(); else delete (x as Step).group })} /></F>
      <p className="text-muted-foreground">신원에 IdMS 로그인(creds login/loginPw)이 있어야 하고 대상에 subscriber 노드(CSC api)가 있어야 한다 — 컴파일 검사가 잡는다</p></>}
    {s.step === 'fd_recv' && <F label="도착 뒤 동작 (payload)" help="download = who 전원이 FILEURL 을 GET(Bearer)으로 내려받아 Metadata size 와 대조(fd_download_pct·fd_download_ms) · signal = FD SIGNALLING 도착만(로그인 불필요)"><Sel value={s.payload ?? 'download'} disabled={ro} options={[{ v: 'download', l: 'download — 내려받아 크기 대조' }, { v: 'signal', l: 'signal — 도착만' }]} onChange={v => ST(x => { if (v === 'signal') x.payload = 'signal'; else delete x.payload })} /></F>}
