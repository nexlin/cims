...
def build_plan(scenario: Scenario, topology: Topology, topology_doc: dict, profile: Optional[LoadProfile],
    ...
    for issue in KNOWN_TARGET_ISSUES:
        try:
            ...
            pass
    # 역할 → 그 창의 신원 사용자부(비밀 없음, 워커 배정 순 — [0] 이 단발 첫 인스턴스의 배정) = 컴파일이 준 것. 픽스처도 같은 신원에 입는다
    by_role = plan.get('role_identities') or {}
    fixtures = plan.get('fixtures') or []
    if fixtures:
        out['notes'].append('픽스처(run 직전 대상 CSC 관리 API 로 적용·확인, 끝나면 복원): ' + ' · '.join(tester_fixtures.summarize(fixtures)))
    out.update({
