...
발견: 토폴로지 `workers` 수동 항목(B 단계). 자기 base 배포 목록(`GET /api/v1/deployments`)에서의
자동 발견은 base 토큰 위임이 정리되면 여기에 보탠다(C 단계).
...
class WorkerClient:
    def __init__(self, name: str, url: str, cpus: Optional[int] = None):
        self.name = name
        self.url = url.rstrip('/')
        self.cpus = cpus
    ...
    def to_dict(self) -> dict:
        return {'name': self.name, 'url': self.url, 'cpus': self.cpus, 'health': self.health,
                'error': self.health_error, 'up': self.health is not None}


def discover(topology_doc: dict) -> List[WorkerClient]:
    out = []
    for w in (topology_doc.get('workers') or []):
        if not isinstance(w, dict) or not w.get('url'):
            continue
        out.append(WorkerClient(str(w.get('name') or w['url']), str(w['url']), w.get('cpus')))
    return out
