...
CIMS_RECORDING_API_DOCS = [
    {'id': 'recording.list', 'module': 'oam-svc', 'method': 'GET', 'path': '/api/v1/recordings',
     'summary': '녹취 세션 목록 (VoIP/PTT, 기간·발신자·그룹 필터, 페이지네이션)',
     'params': [
     ...
     'response': '{total, limit, offset, recordings[]}',
     'response_fields': [
         {'name': 'total', 'type': 'integer', 'unit': '건', 'desc': '필터 적용 후 전체 건수'},
         {'name': 'limit', 'type': 'integer', 'desc': '적용된 페이지 크기'},
         {'name': 'offset', 'type': 'integer', 'desc': '적용된 오프셋'},
         ...
         {'name': 'recordings[].start_time', 'type': 'string', 'desc': 'ISO8601 시작 시각'},
         {'name': 'recordings[].end_time', 'type': 'string', 'desc': 'ISO8601 종료 시각 (진행 중이면 null)'},
         {'name': 'recordings[].duration', 'type': 'integer', 'unit': '초', 'desc': '통화시간'},
         {'name': 'recordings[].state', 'type': 'string', 'desc': '세션 상태'},
     ],
     'example': {'total': 1, 'limit': 200, 'offset': 0,
                 'recordings': [{'id': 'voip/2026/07/30/09/010/01000000001/S20260730091122_1.d',
                                 'call_type': 'volte', 'caller': '01000000001',
                                 'callee': '01000000002', 'group_id': None,
                                 'start_time': '2026-07-30T09:11:22',
                                 'end_time': '2026-07-30T09:12:44', 'duration': 82,
                                 'state': 'ended'}]},
     'errors': list(_ERR_COMMON),
     'notes': ['DB 가 아니라 **파일시스템 스캔**이다 — 기간을 넓게 잡으면 느려진다.',
               'limit 은 1000 을 넘겨도 1000 으로 절삭된다.',
               'id 를 그대로 URL path 에 붙이면 404 가 난다 — 반드시 URL-encode 할 것.'],
     'auth': dict(_AUTH_MONITOR)},

    {'id': 'recording.get', 'module': 'oam-svc', 'method': 'GET', 'path': '/api/v1/recordings/{id}',
     'summary': '녹취 세션 1건 상세 — 메타 + 세그먼트 목록 + 발언 통계',
     'params': [{'name': 'id', 'in': 'path', 'type': 'string', 'required': True,
                 'desc': '세션 식별자 (recording.list 의 id, **URL-encode**)'}],
     'response': '{id, call_type, caller, callee, group_id, start_time, end_time, duration, state, '
                 'segments[], segment_count, total_speech_ms, turn_count, max_concurrent, has_video, status}',
     'response_fields': [
         ...
         {'name': 'turn_count', 'type': 'integer', 'unit': '회',
          'desc': '발언 턴 수 — 동시 발언 세그먼트는 턴이 여럿이라 segment_count 와 다르다'},
         {'name': 'max_concurrent', 'type': 'integer', 'unit': '명', 'desc': '동시 발언 최대 인원'},
         {'name': 'has_video', 'type': 'boolean', 'desc': '영상 트랙 포함 여부'},
         {'name': 'status', 'type': 'string', 'enum': ['recording', 'raw', 'transcoding', 'ready', 'failed'],
          'desc': '재생 가능 상태 — ready 만 즉시 재생 가능'},
         {'name': 'segments[]', 'type': 'object', 'desc': 'recording.segments 와 동일 구조'},
     ],
     'example': {'id': 'voip/2026/07/30/09/010/01000000001/S20260730091122_1.d',
                 'call_type': 'volte', 'caller': '01000000001', 'callee': '01000000002',
                 'group_id': None, 'start_time': '2026-07-30T09:11:22',
                 'end_time': '2026-07-30T09:12:44', 'duration': 82, 'end_reason': 'normal',
                 'segment_count': 4, 'total_speech_ms': 61200, 'turn_count': 4,
                 'max_concurrent': 1, 'has_video': False, 'status': 'ready', 'segments': []},
     'errors': _ERR_COMMON + [
         {'status': 404, 'when': '세션 디렉터리 없음', 'body': {'error': 'Not found'}},
         {'status': 404, 'when': '디렉터리는 있으나 메타 파일 없음', 'body': {'error': 'No metadata'}},
     ],
     'notes': ['status 가 raw/transcoding 이면 재생 요청 시 on-demand 변환이 큐잉되고 곧바로 202 가 온다.'],
     'auth': dict(_AUTH_MONITOR)},

    {'id': 'recording.delete', 'module': 'oam-svc', 'method': 'DELETE', 'path': '/api/v1/recordings/{id}',
     'summary': '녹취 세션 삭제 (디렉터리 전체 제거)',
     'params': [{'name': 'id', 'in': 'path', 'type': 'string', 'required': True,
                 'desc': '세션 식별자 (**URL-encode**)'}],
     'response': '{id, deleted}',
     'response_fields': [
         {'name': 'id', 'type': 'string', 'desc': '삭제한 세션 식별자'},
         {'name': 'deleted', 'type': 'boolean', 'desc': '항상 true (실패는 4xx/5xx)'},
     ],
     'example': {'id': 'voip/2026/07/30/09/010/01000000001/S20260730091122_1.d', 'deleted': True},
     'errors': _ERR_COMMON + [
         {'status': 404, 'when': '세션 디렉터리 없음', 'body': {'error': 'Not found'}},
     ],
     'notes': ['원본 RTP·변환 파일까지 함께 지운다 — 되돌릴 수 없다.'],
     'auth': dict(_AUTH_MANAGER)},

    {'id': 'recording.segments', 'module': 'oam-svc', 'method': 'GET',
     'path': '/api/v1/recordings/{id}/segments',
     'summary': '세션의 발언 구간(세그먼트) 목록 — 화자·슬롯 트랙·변환 상태',
     'params': [{'name': 'id', 'in': 'path', 'type': 'string', 'required': True,
                 'desc': '세션 식별자 (**URL-encode**)'}],
     'response': '{id, segments[]}',
     'response_fields': [
         {'name': 'segments[].seq', 'type': 'integer', 'desc': '세그먼트 순번 (재생 API 의 seq)'},
         {'name': 'segments[].type', 'type': 'string', 'desc': '세그먼트 종류'},
         {'name': 'segments[].speaker_id', 'type': 'string', 'desc': '화자 식별자'},
         {'name': 'segments[].caller', 'type': 'string', 'desc': '발신 측'},
         {'name': 'segments[].callee', 'type': 'string', 'desc': '착신 측'},
         {'name': 'segments[].start_time', 'type': 'string', 'desc': 'ISO8601 시작'},
         {'name': 'segments[].end_time', 'type': 'string', 'desc': 'ISO8601 종료'},
         {'name': 'segments[].duration_ms', 'type': 'integer', 'unit': 'ms', 'desc': '길이'},
         {'name': 'segments[].has_video', 'type': 'boolean', 'desc': '영상 포함'},
         {'name': 'segments[].file_size', 'type': 'integer', 'unit': 'byte', 'desc': '변환 파일 크기 (미변환 0)'},
         {'name': 'segments[].status', 'type': 'string',
          'enum': ['recording', 'raw', 'transcoding', 'ready', 'failed'], 'desc': '변환 상태'},
         {'name': 'segments[].status_reason', 'type': 'string', 'desc': 'failed 일 때 사유'},
         {'name': 'segments[].talker_count', 'type': 'integer', 'unit': '명', 'desc': '오디오 트랙(화자) 수'},
         {'name': 'segments[].max_concurrent', 'type': 'integer', 'unit': '명', 'desc': '동시 발언 최대'},
         {'name': 'segments[].speaker_ids', 'type': 'array', 'desc': '화자 목록'},
         {'name': 'segments[].tracks[].slot', 'type': 'integer', 'desc': '슬롯 번호 (동시 발언 분리)'},
         {'name': 'segments[].tracks[].kind', 'type': 'string', 'enum': ['audio', 'video'], 'desc': '트랙 종류'},
         {'name': 'segments[].tracks[].side', 'type': 'string', 'desc': '어느 leg 인지 (a/b 등)'},
         {'name': 'segments[].tracks[].codec', 'type': 'string', 'desc': '코덱 (AMR-WB/H264 등)'},
         {'name': 'segments[].tracks[].pt', 'type': 'integer', 'desc': 'RTP payload type'},
         {'name': 'segments[].tracks[].speakers', 'type': 'array', 'desc': '이 슬롯의 화자'},
         {'name': 'segments[].audio_codec', 'type': 'string', 'desc': '오디오 코덱 메타 (구 녹취는 빈 값)'},
     ],
     'example': {'id': 'voip/2026/07/30/09/010/01000000001/S20260730091122_1.d',
                 'segments': [{'seq': 1, 'type': 'speech', 'speaker_id': '01000000001',
                               'caller': '01000000001', 'callee': '01000000002',
                               'start_time': '2026-07-30T09:11:25',
                               'end_time': '2026-07-30T09:11:40', 'duration_ms': 15000,
                               'has_video': False, 'file_size': 48000, 'status': 'ready',
                               'status_reason': '', 'talker_count': 1, 'max_concurrent': 1,
                               'speaker_ids': ['01000000001'],
                               'tracks': [{'slot': 0, 'kind': 'audio', 'side': 'a', 'pt': 99,
                                           'codec': 'AMR-WB', 'speakers': ['01000000001'],
                                           'has_video': False, 'status': 'ready'}],
                               'audio_codec': 'AMR-WB'}]},
     'errors': _ERR_COMMON + [
         {'status': 404, 'when': '세션 없음', 'body': {'error': 'Not found'}},
     ],
     'notes': ['단일 화자 세그먼트도 tracks 원소가 1개 있다 — 항상 배열로 처리할 것.',
               '변환기 내부 키(`_` 로 시작)는 응답에서 제외된다.'],
     'auth': dict(_AUTH_MONITOR)},

    {'id': 'recording.segment.audio', 'module': 'oam-svc', 'method': 'GET',
     'path': '/api/v1/recordings/{id}/segments/{seq}/{media}',
     'summary': '세그먼트 미디어 스트리밍 (raw RTP → mp4/wav on-demand 변환 후 전송)',
     'params': [
         {'name': 'id', 'in': 'path', 'type': 'string', 'required': True, 'desc': '세션 식별자 (**URL-encode**)'},
     ...
     'response': '오디오/비디오 바이너리 스트림 (JSON 아님)',
     'response_fields': [
         {'name': '(body)', 'type': 'binary', 'desc': 'Content-Type 은 audio/wav 또는 video/mp4'},
     ],
     'errors': _ERR_COMMON + [
         {'status': 202, 'when': '아직 변환 전 — 변환 작업을 큐잉하고 즉시 반환. 잠시 후 재요청'},
         {'status': 404, 'when': '세션/세그먼트 없음', 'body': {'error': 'Not found'}},
     ],
     'notes': ['**202 를 성공으로 취급하되 본문이 미디어가 아님**을 유의 — 폴링으로 재요청해야 한다.',
               '변환은 워커 풀(기본 2)에서 수행되어 동시 변환 수가 제한된다.',
               '변환 결과는 캐싱되어 재요청은 즉시 응답한다.'],
     'auth': dict(_AUTH_MONITOR)},

    {'id': 'recording.audio', 'module': 'oam-svc', 'method': 'GET', 'path': '/api/v1/recordings/{id}/audio',
     'summary': '세션 전체 오디오 스트리밍 (세그먼트 병합 재생)',
     'params': [{'name': 'id', 'in': 'path', 'type': 'string', 'required': True,
                 'desc': '세션 식별자 (**URL-encode**)'}],
     'response': '오디오 바이너리 스트림 (JSON 아님)',
     'response_fields': [
         {'name': '(body)', 'type': 'binary', 'desc': 'Content-Type: audio/wav'},
     ],
     'errors': _ERR_COMMON + [
         {'status': 202, 'when': '변환 전 — 큐잉 후 즉시 반환'},
         {'status': 404, 'when': '세션 없음', 'body': {'error': 'Not found'}},
     ],
     'notes': ['동시 발언 구간은 믹스되어 한 트랙으로 들린다 — 화자 분리 재생은 세그먼트 단위 API 를 쓴다.'],
     'auth': dict(_AUTH_MONITOR)},
]
