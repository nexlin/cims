...
async def _heartbeat(handler_args: HandlerArgs, config: dict, agent: dict) -> HandlerResult:
    ...
    if body.get("hostname"):
        patches['hostname'] = str(body["hostname"]).strip()[:128]
    if body.get("os_info"):
        patches['os_info'] = str(body["os_info"]).strip()[:255]
    for k in ("cpu_cores", "memory_mb", "disk_gb"):
        try:
...
async def _metric(handler_args: HandlerArgs, config: dict, agent: dict) -> HandlerResult:
    ...
    cert_renew = body.get("cert_renew") or {}          # {module: {ok, reason, days_left, ts}} — lifecycle 엔진 leaf 갱신 결과
    record = {
        ...
        'cert_renew': cert_renew if isinstance(cert_renew, dict) else {},
    }
