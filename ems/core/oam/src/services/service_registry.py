...
_CORE_ALERT_RULES = [
    ...
    # 호스트 자원 임계 — 같은 조건 클래스(A-QOS-001)의 다른 객체(§3.5 '새 리소스 = rule 만 추가'): cpu · mem · load · 루트 외 마운트.
    #   mem 은 OOM 선행 지표(csc OOM 사망이 무알람이던 실측). load 는 코어 수로 정규화한 1분 load(%) — I/O wait 포화를 cpu_pct 와 별개로 잡는다.
    {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'perceived_severity': 'warning',
     'event_type': 'qualityOfService', 'probable_cause': 'cpuCyclesLimitExceeded', 'mo_class': 'host',
     'check': 'cpu_high', 'scope': 'agent', 'unit': '%', 'metric': 'CPU 사용률',
     'thresholds': {'minor': 85, 'major': 95},
     'msg_open': '{mo} CPU 사용률 {pct}% ({threshold}% 초과)', 'msg_close': '{mo} CPU 사용률 {pct}% (정상)',
     'effect': '호처리 지연·타임아웃 위험(제어평면 과부하 선행 지표)',
     'recommended_action': '상위 프로세스 확인(모듈별 cpu_pct), 부하 분산·증설'},
    {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'perceived_severity': 'warning',
     'event_type': 'qualityOfService', 'probable_cause': 'outOfMemory', 'mo_class': 'host',
     'check': 'mem_high', 'scope': 'agent', 'unit': '%', 'metric': '메모리 사용률',
     'thresholds': {'minor': 85, 'major': 92, 'critical': 97},
     'msg_open': '{mo} 메모리 사용률 {pct}% ({threshold}% 초과)', 'msg_close': '{mo} 메모리 사용률 {pct}% (정상)',
     'effect': 'OOM killer 로 모듈 사망 위험',
     'recommended_action': '모듈별 RSS 확인(누수 의심 시 재기동), 메모리 증설'},
    {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'perceived_severity': 'minor',
     'event_type': 'qualityOfService', 'probable_cause': 'performanceDegraded', 'mo_class': 'host',
     'check': 'load_high', 'scope': 'agent', 'unit': '% of cores', 'metric': 'load average(1분, 코어 대비)',
     'thresholds': {'minor': 200, 'major': 400},
     'msg_open': '{mo} load {load} = 코어 대비 {pct}% ({threshold}% 초과)', 'msg_close': '{mo} load {load} (정상)',
     'effect': 'run-queue/I/O wait 포화 — 미디어 지터·시그널링 지연',
     'recommended_action': 'I/O wait·디스크(NAS) 응답 확인, 부하 분산'},
    {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'perceived_severity': 'warning',
     'event_type': 'qualityOfService', 'probable_cause': 'storageCapacityProblem', 'mo_class': 'host',
     'check': 'mount_high', 'scope': 'agent', 'unit': '%', 'metric': '마운트 사용률',
     'thresholds': {'minor': 80, 'major': 90, 'critical': 95},
     'msg_open': '{mo} 사용률 {pct}% ({threshold}% 초과)', 'msg_close': '{mo} 사용률 {pct}% (정상)',
     'effect': '녹취/로그/데이터 볼륨 용량 임계 근접 — 적재 실패 위험',
     'recommended_action': '오래된 파일 정리, 볼륨 증설 (루트 외 로컬 마운트 — NAS 는 수집 밖)'},
...
_ALERT_CLASS_DEFAULTS = {
    ...
    'mount_high':   {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'event_type': 'qualityOfService',
                     'probable_cause': 'storageCapacityProblem', 'mo_class': 'host', 'perceived_severity': 'warning'},
    'cpu_high':     {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'event_type': 'qualityOfService',
                     'probable_cause': 'cpuCyclesLimitExceeded', 'mo_class': 'host', 'perceived_severity': 'warning'},
    'mem_high':     {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'event_type': 'qualityOfService',
                     'probable_cause': 'outOfMemory', 'mo_class': 'host', 'perceived_severity': 'warning'},
    'load_high':    {'type': 'capacity_threshold', 'code': 'A-QOS-001', 'event_type': 'qualityOfService',
                     'probable_cause': 'performanceDegraded', 'mo_class': 'host', 'perceived_severity': 'minor'},
