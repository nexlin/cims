...
export interface EventFilter {
  kind: string; type: string; q: string; page: number
}

const INIT_ALARM: AlarmFilter = { sev: '', code: '', type: '', q: '', showResolved: true, page: 0 }
const INIT_EVENT: EventFilter = { kind: '', type: '', q: '', page: 0 }
...
export const alertsFilter = {
  ...
  setAlarm(patch: Partial<AlarmFilter>) {
    alarmF = { ...alarmF, ...patch, ...(('page' in patch) ? {} : { page: 0 }) }
    notify()
  },
  setEvent(patch: Partial<EventFilter>) {
    eventF = { ...eventF, ...patch, ...(('page' in patch) ? {} : { page: 0 }) }
    notify()
  },
}
