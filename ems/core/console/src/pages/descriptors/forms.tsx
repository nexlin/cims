...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <label className="flex flex-col gap-[3px] text-sm">
      <span className="text-muted-foreground">{label}{hint && <i className="ml-1.5 opacity-70">{hint}</i>}</span>
      {children}
    </label>
  )
}
function Btn({ onClick, children, danger, disabled }: { onClick: () => void; children: React.ReactNode; danger?: boolean; disabled?: boolean }) {
  return <Button type="button" disabled={disabled}
    className={danger ? 'text-destructive' : undefined} onClick={onClick}>{children}</Button>
}
  ...
  return (
    <Modal title={index == null ? `모듈 추가 — ${svc.id}` : `모듈 편집 — ${cur?.name}`} onClose={onClose} width={460}>
      <div className="flex gap-2.5 flex-wrap items-end">
        <Field label="이름"><Input  className="w-[140px]" value={m.name}
          onChange={e => up({ name: e.target.value })} /></Field>
        <Field label="포트"><Input  className="w-[90px]" type="number" value={m.port ?? ''}
...
export function AlertRuleForm({ svc, index, onClose, onSaved }: {
  ...
  <SelectContent>
                ...
                <Field label="code"><Input  className="w-[110px]" value={r.code ?? ''}
  ...
  <SelectContent>
                ...
                <Field label="probable_cause"><Input  className="w-[160px]" value={r.probable_cause ?? ''}
  ...
  <SelectContent>
                ...
                <Field label="mo_instance" hint="(소스, service)"><Input  className="w-[120px]" value={r.mo_instance ?? ''}
              ...
              <div className="flex gap-2 mt-1.5 flex-wrap">
                <Field label="영향(effect)"><Input  className="w-[240px]" value={r.effect ?? ''}
                  onChange={e => up({ effect: e.target.value })} /></Field>
                <Field label="권장 조치(action)"><Input  className="w-[240px]" value={r.recommended_action ?? ''}
    ...
    <Modal title={index != null ? `데이터 소스 편집 — ${existing?.id}` : '데이터 소스 추가'} onClose={onClose} width={680}>
      <div className="flex max-h-[70vh] flex-col gap-3.5 overflow-y-auto">
        <div className="flex gap-3 flex-wrap">
          <Field label="id"><Input  className="w-[180px]" value={id} onChange={e => setId(e.target.value)} /></Field>
          <Field label="표시명"><Input  className="w-[160px]" value={label} onChange={e => setLabel(e.target.value)} /></Field>
        </div>
        <div className="flex gap-3 items-end flex-wrap">
          <Field label="endpoint" hint="(REST 경로)"><Input  className="w-[300px]" value={endpoint}
        ...
        {/* shape별 매핑 */}
        {shapes.has('time-bar') && (
          <div className="mb-2 rounded-md border border-border bg-muted p-2.5">
            <b className="text-sm">시계열 차트 매핑</b>
            <div className="flex gap-2 mt-1.5 flex-wrap">
              <Field label="from" hint="(배열 경로)"><Input  className="w-[160px]" value={tbFrom} onChange={e => setTbFrom(e.target.value)} placeholder="buckets / voip.buckets" /></Field>
              <Field label="label" hint="(필드 후보, 쉼표)"><Input  className="w-[130px]" value={tbLabel} onChange={e => setTbLabel(e.target.value)} placeholder="hour, date" /></Field>
              <Field label="value"><Input  className="w-[110px]" value={tbValue} onChange={e => setTbValue(e.target.value)} placeholder="count" /></Field>
            </div>
          </div>
        )}
        {shapes.has('kpi') && (
          <div className="mb-2 rounded-md border border-border bg-muted p-2.5">
            ...
            {kpiItems.map((k, i) => (
              <div className="flex gap-2 mt-1.5 items-end" key={i}>
                <Field label="라벨"><Input  className="w-[110px]" value={k.label} onChange={e => upKpi(i, { label: e.target.value })} /></Field>
                <Field label="path"><Input  className="w-[160px]" value={k.path} onChange={e => upKpi(i, { path: e.target.value })} placeholder="voip.total_attempts" /></Field>
                <Field label="단위"><Input  className="w-[50px]" value={k.unit ?? ''} onChange={e => upKpi(i, { unit: e.target.value })} /></Field>
  ...
  <SelectContent>
        ...
        {shapes.has('distribution') && (
          <div className="mb-2 rounded-md border border-border bg-muted p-2.5">
            ...
            <div className="flex gap-2 mt-1.5 flex-wrap">
              <Field label="fromObject" hint="(dict 경로)"><Input  className="w-[180px]" value={distObj} onChange={e => setDistObj(e.target.value)} placeholder="voip.end_reasons" /></Field>
              <Field label="totalPath" hint="(분모)"><Input  className="w-[180px]" value={distTotal} onChange={e => setDistTotal(e.target.value)} placeholder="voip.total_attempts" /></Field>
            </div>
          </div>
        )}
        {shapes.has('table') && (
          <div className="mb-2 rounded-md border border-border bg-muted p-2.5">
            ...
            <div className="flex gap-2 mt-1.5 flex-wrap">
              <Field label="fromObject" hint="(dict 경로)"><Input  className="w-[180px]" value={tblObj} onChange={e => setTblObj(e.target.value)} placeholder="method_counts" /></Field>
              <Field label="컬럼" hint="(키, 값 — 쉼표)"><Input  className="w-[160px]" value={tblCols} onChange={e => setTblCols(e.target.value)} placeholder="메서드, 건수" /></Field>
            </div>
          </div>
        )}
      </div>
