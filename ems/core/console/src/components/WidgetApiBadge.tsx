...
function CopyBlock({ label, text, mono, pre }: {
  ...
  return (
    <div className="mb-2">
      ...
      {pre ? (
        <pre className="m-0 py-2 px-2.5 bg-background border border-border rounded-sm text-xs overflow-x-auto whitespace-pre">{text}</pre>
      ) : (
  ...
  return (
    <div className="border-b border-border py-2.5 px-0">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <code className="text-sm break-all">{a.path}</code>
                ...
                <tbody>
                  {errors.map((e, i) => (
                    <tr key={`${e.status}-${i}`}>
                      ...
                      <Td>{e.when || ''}</Td>
                      <Td>{e.body !== undefined
                        ? <code className="text-xs">{JSON.stringify(e.body)}</code> : '—'}</Td>
                    </tr>
                  ))}
                </tbody>
              </DataTable>
