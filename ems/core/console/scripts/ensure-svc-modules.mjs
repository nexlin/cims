// ems/{service,tester}/console/node_modules → ../../core/console/node_modules 링크 보증.
//
// 콘솔 팩(service/console·tester/console)은 자체 node_modules 없이 core/console 의 것을
// 심볼릭 링크로 공유한다 (jsx: react-jsx 가 각 소스 파일 위치 기준으로 react/jsx-runtime 을
// 상향 탐색하므로, 팩 src 위쪽에 react 가 보여야 tsc 가 통과).
...
const PACKS = ['service', 'tester']   // ems/<pack>/console — 새 팩은 여기 한 줄
...
for (const pack of PACKS) {
  const packDir = path.resolve(here, '..', '..', '..', pack, 'console')
  if (!fs.existsSync(packDir)) continue              // 팩 없는 배포본 — 무시
  const link = path.join(packDir, 'node_modules')
