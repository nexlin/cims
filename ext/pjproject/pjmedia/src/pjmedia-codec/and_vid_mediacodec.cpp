 ...
 * CIMS: 2초 IDR 주기(대역폭·복구 균형). 인코더 i-frame-interval 로 반영(and_media_codec
 * 디스크립터의 keyframe_interval 필드는 미사용 dead field). */
#define KEYFRAME_INTERVAL       2
...
static pj_status_t configure_encoder(and_media_codec_data *and_media_data)
{
    ...
    /* CIMS: 발신 영상 비트레이트 상한 500kbps. PJSIP 이 협상 시 해상도 기반으로 재계산한
     * 과도한 기본값(예: 480x640@15 → ~920kbps)을 캡해 대역폭을 억제한다. */
    {
        ...
        /* CBR 모드 — Android MediaCodec 의 min-quality(VQApply) 기능이 VBR 에서 저비트레이트를
         * 품질 floor(예: 480x640@15 → 921kbps)로 강제 상향하는 것을 방지(목표 비트레이트 준수). */
