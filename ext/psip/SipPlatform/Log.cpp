...
/**
 * @ingroup SipPlatform
 * @brief 로그 파일 이름에 붙일 prefix(프로세스 이름)를 설정한다.
 *        SetDirectory 호출 전에 설정해야 한다.
 * @param pszPrefix 프로세스 이름 (예: "csp", "cwrtc")
 */
void CLog::SetPrefix( const char * pszPrefix )
{
	if( pszPrefix == NULL ) return;
	snprintf( m_szPrefix, sizeof(m_szPrefix), "%s", pszPrefix );
}
...
OPEN_FILE:
		...
		if( m_szPrefix[0] != '\0' )
			snprintf( szFileName, sizeof(szFileName), "%s%c%s_%04d%02d%02d_%d.log", m_pszDirName, DIR_SEP, m_szPrefix, sttTm.tm_year + 1900, sttTm.tm_mon + 1, sttTm.tm_mday, m_iIndex );
		else
