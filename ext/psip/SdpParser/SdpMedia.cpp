...
// FMT 리스트에 payload type 이 존재하면 해당 payload type 을 FMT 리스트 및 애트리뷰트에서 삭제한다.
...
// FMT 리스트에 payload type 이 존재하면 해당 payload type 을 FMT 리스트 및 애트리뷰트의 최상단으로 이동한다.
...
// 애트리뷰트를 추가한다. rtpmap 이면 fmt 로 추가한다.
