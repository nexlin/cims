...
// XML 에 저장된 element 리스트를 문자열 맵 자료구조에 저장한다.
...
// 설정 파일에서 SIP 서버 리스트를 제외한 실시간으로 수정 가능한 항목을 읽는다.
...
// 설정 파일에서 SipServer 정보를 읽어서 SipServerMap 에 저장한다.
