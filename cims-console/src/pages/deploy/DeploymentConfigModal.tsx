...
export default function DeploymentConfigModal({ deployment, onClose, onDone }: {
  ...
  const load = useCallback(async () => {
    try {
      const r = await deploymentApi.getDeploymentConfig(deployment.id)
      setTemplate(r.template)
      setAppliedAt(r.config_applied_at)
      // 기존 값 + 템플릿 default 로 초기화
      ...
      // template 에 없는데 config 에만 있는 값도 보존
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      const r = await deploymentApi.putDeploymentConfig(deployment.id, values, true)
      show(r.job_id ? `저장됨. update_config job #${r.job_id}` : '저장됨', 'ok')
      await onDone()
      onClose()
    } catch (e) {
      show((e as Error).message, 'err')
  ...
  return (
    <Modal title={`${deployment.package_name} v${deployment.package_version} — 설정`}
        ...
        <>
          ...
          {tab === 'scalar' ? (
            ...
            (() => {
              ...
              return <CollectionEditor deploymentId={deployment.id} collection={coll} />
            })()
          )}
        </>
      )}
