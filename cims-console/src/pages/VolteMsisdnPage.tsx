...
export default function VolteMsisdnPage() {
  const { show } = useToast()
  ...
  const subs = selectedUser?.call_subscriptions || []
  ...
  async function saveEdit() {
    ...
    try { await usersApi.updateSub(selectedUser.id,'call',editMsisdn,d); show('수정','ok'); setEditMsisdn(null); load() } catch(e:unknown){show(String(e),'err')}
  }
  function startAdd() {
  ...
  async function saveAdd() {
    ...
    try { await usersApi.addSub(selectedUser.id,'call',addForm); show('추가','ok'); setAdding(false); load() } catch(e:unknown){show(String(e),'err')}
  }
  async function handleDel(msisdn:string) {
    if(!selectedUser||!confirm(`${msisdn} 삭제?`)) return
    try { await usersApi.deleteSub(selectedUser.id,'call',msisdn); show('삭제','ok'); load() } catch(e:unknown){show(String(e),'err')}
  }
    ...
    <div style={{ display:'flex', gap:16, alignItems:'flex-start' }}>
      ...
      <div style={{ flex:1 }}>
        {!selectedUser ? (
          ...
          <>
            <div style={{ marginBottom:16, fontWeight:600, fontSize:15 }}>{selectedUser.name}의 VoLTE 번호</div>
